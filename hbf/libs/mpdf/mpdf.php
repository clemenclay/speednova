<?php
/**
 * modified on 2016-10-06 - added php7 support
 * @todo: test updat to https://github.com/mpdf/mpdf
 */
// ******************************************************************************
	...
	if ($this->ColActive || !$this->keep_block_together || $this->blk[$this->blklvl]['startpage'] == $this->page || ($this->page - $this->blk[$this->blklvl]['startpage']) > 1 || ($height[$p1]+$height[$p2]) > $this->h) { 
	   ...
	   foreach($this->divbuffer AS $key=>$s) { 
		...
		if (preg_match('/(___PAGE___START'.date('jY').')/', $t)) { $ispre = false; }
		  ...
		  $t = preg_replace_callback('/BT (\d+\.\d\d+) (\d+\.\d\d+) Td/',
              function($matches) use($k,$p,$xadj,$yadj) {
                  return $this->blockAdjust('Td',$k,$xadj[$p],$yadj[$p],$matches[1],$matches[2]);
              },
              $t);
		  $t = preg_replace_callback('/(\d+\.\d\d+) (\d+\.\d\d+) (\d+\.\d\d+) ([\-]{0,1}\d+\.\d\d+) re/',
              function($matches) use($k,$p,$xadj,$yadj) {
                  return $this->blockAdjust('re',$k,$xadj[$p],$yadj[$p],$matches[1],$matches[2],$matches[3],$matches[4]);
              },
              $t);
		  $t = preg_replace_callback('/(\d+\.\d\d+) (\d+\.\d\d+) l/',
              function($matches) use($k,$p,$xadj,$yadj) {
                  return $this->blockAdjust('l',$k,$xadj[$p],$yadj[$p],$matches[1],$matches[2]);
              },
              $t);
		  $t = preg_replace_callback('/q (\d+\.\d\d+) 0 0 (\d+\.\d\d+) (\d+\.\d\d+) (\d+\.\d\d+) cm \/(I|FO)/',
              function($matches) use($k,$p,$xadj,$yadj) {
                  return $this->blockAdjust('img',$k,$xadj[$p],$yadj[$p],$matches[1],$matches[2],$matches[3],$matches[4],$matches[5]);
              },
              $t);
		  $t = preg_replace_callback('/(\d+\.\d\d+) (\d+\.\d\d+) m/',
              function($matches) use($k,$p,$xadj,$yadj) {
                  return $this->blockAdjust('draw',$k,$xadj[$p],$yadj[$p],$matches[1],$matches[2]);
              },
              $t);
		  $t = preg_replace_callback('/(\d+\.\d\d+) (\d+\.\d\d+) (\d+\.\d\d+) (\d+\.\d\d+) (\d+\.\d\d+) (\d+\.\d\d+) c/',
              function($matches) use($k,$p,$xadj,$yadj) {
                  return $this->blockAdjust('bezier',$k,$xadj[$p],$yadj[$p],$matches[1],$matches[2],$matches[3],$matches[4],$matches[5],$matches[6]);
              },
              $t);
		}
	      if ($ispre) $pre .= $t."\n";
	...
	if ($usepre) //used to keep \n on content inside <pre> and inside <textarea>
 	{
		...
		while($thereispre) //Recover <pre attributes>content</pre>
		{
			$temp[2][$iterator] = preg_replace_callback("/^([^\n\t]*?)\t/m",
                function($matches) use ($tabSpaces) {
                    return stripslashes($matches[1]) . str_repeat(' ',  ( $tabSpaces - (mb_strlen(stripslashes($matches[1])) % $tabSpaces))  );
                } ,$temp[2][$iterator]);
