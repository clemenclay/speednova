 ...
 */
  ...
  /**
   * NOT WORKING * Check that a string looks roughly like an email address should
  ...
  public static function ValidateAddress($address) {
    return preg_match('/^.*@.*$/s', $address);
    /*
    * produces issues on some servers
  ...
  public function Send() {
    $excpt = $this->exceptions;
    $this->exceptions = true;
    try { 
      if(!$this->PreSend()) 
          return false;
      $result = $this->PostSend();
      $this->exceptions = $excpt;
      return $result;
  ...
  protected function MailSend($header, $body) {
    ...
    if (empty($this->Sender)) {
      $params = "-oi ";
    } elseif($this->validateAddress($this->Sender)) {
  ...
  public function SmtpConnect() {
    ...
    try {
      while($index < count($hosts) && !$connection) {
        ...
        $proto = explode("://", $host);
        if(count($proto) > 1 && in_array(strtolower($proto[0]), array('ssl', 'tls'))) {
            $this->SMTPSecure = strtolower($proto[0]);
            $host = $proto[1];
        }
  ...
  */
  ...
  protected function SetError($msg) {
    ...
    if ($this->Mailer == 'smtp' and !is_null($this->smtp)) {
       ...
       */
      $errors = $this->smtp->getErrors();
      if (!empty($errors)) {
        foreach($errors as $error){
          if (!empty($error) && array_key_exists('error', $error)) {
            $msg .= '<p>' . $this->Lang('smtp_error') . $error['error'] . "</p>\n";
          }
        }
      }
    }
