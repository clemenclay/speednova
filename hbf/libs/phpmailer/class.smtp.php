...
class SMTP {
  ...
  private $error;
  /**
   * @var array Error messages, if any
   */
  private $errors;
  /**
  ...
  public function Authenticate($username, $password, $authtype='LOGIN', $realm='', $workstation='') {
    ...
    switch ($authtype) {
      ...
      case 'NTLM':
        ...
        if($code != 334) {
            $this->addError(array("error" => "AUTH not accepted from server",
                      "smtp_code" => $code,
                      "smtp_msg" => substr($rply,4)));
            if($this->do_debug >= 1) {
                $this->edebug("SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF);
        ...
        if($code != 235) {
            $this->addError(array("error" => "Could not authenticate",
                      "smtp_code" => $code,
                      "smtp_msg" => substr($rply,4)));
            if($this->do_debug >= 1) {
                $this->edebug("SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF);
  ...
  public function Close() {
    $this->addError(null); // so there is no confusion
    $this->helo_rply = null;
    if(!empty($this->smtp_conn)) {
  ...
  public function Data($msg_data) {
    $this->addError(null); // so no confusion is caused
    ...
    if($code != 354) {
      $this->addError(array("error" => "DATA command not accepted from server",
              "smtp_code" => $code,
              "smtp_msg" => substr($rply,4)));
      if($this->do_debug >= 1) {
        $this->edebug("SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF );
    ...
    if($code != 250) {
      $this->addError(array("error" => "DATA not accepted from server",
              "smtp_code" => $code,
              "smtp_msg" => substr($rply,4)));
      if($this->do_debug >= 1) {
        $this->edebug("SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF );
  ...
  private function SendHello($hello, $host) {
    ...
    if($code != 250) {
      $this->addError(array("error" => $hello . " not accepted from server",
              "smtp_code" => $code,
              "smtp_msg" => substr($rply,4)));
      if($this->do_debug >= 1) {
        $this->edebug("SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF );
  ...
  public function Mail($from) {
    ...
    if($code != 250) {
      $this->addError(array("error" => "MAIL not accepted from server",
              "smtp_code" => $code,
              "smtp_msg" => substr($rply,4)));
      if($this->do_debug >= 1) {
        $this->edebug("SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF );
  ...
  public function Recipient($to) {
    ...
    if($code != 250 && $code != 251) {
      $this->addError(array("error" => "RCPT not accepted from server",
              "smtp_code" => $code,
              "smtp_msg" => substr($rply,4)));
      if($this->do_debug >= 1) {
        $this->edebug("SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF );
  ...
  public function Reset() {
    ...
    if($code != 250) {
      $this->addError(array("error" => "RSET failed",
              "smtp_code" => $code,
              "smtp_msg" => substr($rply,4)));
      if($this->do_debug >= 1) {
        $this->edebug("SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF );
  ...
  public function SendAndMail($from) {
    ...
    if($code != 250) {
      $this->addError(array("error" => "SAML not accepted from server",
              "smtp_code" => $code,
              "smtp_msg" => substr($rply,4)));
      if($this->do_debug >= 1) {
        $this->edebug("SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF );
  ...
  public function Turn() {
    $this->addError(array("error" => "This method, TURN, of the SMTP ".
                                    "is not implemented"));
    if($this->do_debug >= 1) {
