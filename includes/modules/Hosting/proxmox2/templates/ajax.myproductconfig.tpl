{if $make=='getonappval' && $valx}
    {if $valx=='option23'}   {* Node  *}
        ...
        <select id="option23" multiple class="multi" name="options[option23][]">
            <option value="Auto-Assign" {if in_array('Auto-Assign',$defval)}selected="selected"{/if}>All Nodes</option>
    ...
    {elseif $valx=='backupstorage'} {*  backup store zone  *}
    ...
    {elseif $valx=='hagroup'}  {*  HA Group  *}

        <select id="option22"  name="options[hagroup]">
            {foreach from=$modvalues item=g}
                 <option value="{$g.group}" data-nodes="{$g.nodes}" {if $defval == $g.group}selected="selected"{/if}>{$g.group}</option>
            {/foreach}
        </select>
