...
<script type="text/javascript">
    function loadIps() {
        ...
        $.getJSON('{/literal}{$service_url}&vpsdo=ips&vpsid={$vpsid}&do=fetchips{literal}', {network_interface_id:w}, function(data){
            $('#ip_address_id').removeAttr('disabled');
            $('#iploader').hide();
            if(data && data.ips) {
...
<table class="data-table backups-list"  width="100%" cellspacing=0>
    ...
    <tr>
        ...
        <td>{$ip.interface}</td>
        <td>
            <a title="Delete"  href="{$service_url}&vpsdo=ips&vpsid={$vpsid}&do=deleteip&ipid={$ip.id}&security_token={$security_token}" onclick="return  confirm('{$lang.suretodeleteip}?')" class="small_control small_delete fs11">{$lang.delete}</a>
        </td>
    </tr>
    {foreachelse}
    <tr>
...
<div style="padding:10px 0px;text-align:right">
    ...
    {/if}
    <input type="button" class="blue" onclick="if(confirm('{$lang.rebuildconfirm}'))window.location='{$service_url}&vpsdo=ips&vpsid={$vpsid}&do=rebuildnetwork&security_token={$security_token}'" value="{$lang.rebuildnetwork}"/>

    {if $allowaddingip}
    <input type="button" class="blue" onclick="window.location='{$service_url}&vpsdo=ips&vpsid={$vpsid}&do=assignip'" value="{$lang.assign_new_ip}"/>
