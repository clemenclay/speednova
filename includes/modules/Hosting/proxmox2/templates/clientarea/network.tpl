...
<div class="header-bar">
    <h3 class="{$vpsdo} hasicon">
        {if $vpsdo=='ips'}
            {$lang.ips} 
            {if $subdo=='assignip'}&raquo; {$lang.assign_new_ip}{/if}
    ...
    <ul class="sub-ul">
        <li >
            <a href="{$service_url}&vpsdo=interfaces&vpsid={$vpsid}" 
               class="{if $vpsdo=='interfaces'}active{/if}" ><span>{$lang.interfaces}</span></a>
        </li>
        {*}
        <li >
            <a href="{$service_url}&vpsdo=ips&vpsid={$vpsid}" 
               class="{if $vpsdo=='ips'}active{/if}" ><span>{$lang.ips}</span></a>
        </li>
        {*}
        {if $o_sections.o_firewall}
            <li >
                <a  href="{$service_url}&vpsdo=firewall&vpsid={$vpsid}" 
                    class="{if $vpsdo=='firewall'}active{/if}"><span>{$lang.Firewall}</span></a>
            </li>
        {/if}
    </ul>
    <div class="clear"></div>
