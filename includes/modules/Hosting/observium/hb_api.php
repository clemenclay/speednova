...
/**
 * Tested on: Observium CE 0.16.7.8003
 */
...
function hb_get_data($port_id, $from=false, $to=false, $n95th=false) {
    ...
    include_once($config['install_dir'] . "/html/includes/graphs/port/auth.inc.php");
    include_once($config['install_dir'] . "/html/includes/graphs/port/bits.inc.php");
    include_once($config['install_dir'] . "/html/includes/graphs/generic_data.inc.php");
    ob_get_clean();
    ...
    if($n95th) {
        ...
        $ret =$text=rrdtool_95thgraph_xport($rrd_options);
        ob_get_clean();
        ...
        $ret=$xml=rrdtool_graph_xport($rrd_options);
        ob_get_clean();
...
function rrdtool_graph_xport($options) {
    ...
    if (is_resource($rrd_process)) {
        ...
        while (strlen($line) < 1) {
            ...
            usleep(10000);
            $line = stream_get_contents($rrd_pipes[1]);
            $data .= $line;
        }
...
function rrdtool_95thgraph_xport($options) {
    ...
    $xport ="";
    if(preg_match_all("/(?<=\s)VDEF:[\s]?([a-zA-Z0-9]+)(?==)/", $options, $matches)) {
        $def = $matches[1];
        foreach($def as $d) {
            $xport.= "PRINT:" . $d . ":" . "\"" . str_replace(":", "", $d) . "-%lf\" ";
        }
    }
    rrdtool_pipe_open($rrd_process, $rrd_pipes);
    ...
    if (is_resource($rrd_process)) {
        ...
        fwrite($rrd_pipes[0], "graph /dev/null $graph_file $options $xport");
...
class HB_ObserviumAPI {
     ...
     *
     */
    public function getGraphData($switch, $port, $from, $to, $n) {
        //small sanitization
        $from = intval($from);
        $to = intval($to);
