...
class HB_CactiAPI {
    ...
    public function listSwitches() {
        ...
        foreach ($switches as &$switch) {
            $switch['ports'] = $this->listPortsNew($switch['id']);
        }
        return $switches;
    }
    ...
    public function listDeviceGraphs($host_id) {
        $poout = db_fetch_assoc("
          ...
          FROM 
            host h
           JOIN graph_local g ON h.id = g.host_id
           JOIN  graph_templates t ON t.id = g.graph_template_id
           JOIN graph_templates_graph tg ON tg.local_graph_id = g.id
           
          WHERE h.id= '" . $host_id . "' AND tg.title_cache!='' ");
     ...
     * array('11'=>array('ifDescr'=>'Ethernet ...','ifName'=>'1/0/1'))
     */
    private function listPortsNew($host_id) {


        $poout = db_fetch_assoc("SELECT
            data.value,
            fields.data_name,
            local.snmp_index,
            data.data_template_data_id

            FROM
            data_input_data data
            JOIN
            data_input_fields fields ON fields.id = data.data_input_field_id
            JOIN
            data_template_data template ON data.data_template_data_id = template.id
            JOIN
            data_local local ON template.local_data_id=local.id

            WHERE
            fields.data_name IN ('index_value')
            AND
            local.host_id = '" . $host_id . "' ORDER BY local.snmp_index ASC");
        ...
        //transpose rows to cols
        foreach ($poout as $v) {
            $ports[$v['snmp_index']]['ifDescr'] = $v['value'];
            $ports[$v['snmp_index']]['ifName'] = $v['value'];
        }
        return $ports;
    }
        ...
        if (!isset($request['hash'])) {
            ...
            switch ($request['cmd']) {
                case 'listswitches':
                    ...
                    break;
                case 'listdevices':
                    self::$output['devices'] = $api->listDevices();
                    break;
