<div id="meteredmgr" >
    <div style="">
                ...
                <tr>
                    <td valign="top">
                        <div style="padding:10px 0px;">
                            <a class="prices menuitm newmeter" href="#"><span class="addsth">Add new variable for Class of Service</span></a>
                        </div>
                        <small>Before adding new variables, make sure that you setup connection with Zimbra server in "Connect with App" tab</small>
                    </td>
                </tr>
                <tr id="metered_var_editor" style="display:none">
                    <td>
                        <table border="0" cellpadding="3" class="left" cellspacing="0" id="powerdnspanel_table">
                            <tbody><tr>
                                    <td width="100" class="fs11"><span class="left">COS</span> </td>
                                ...
                                <tr>
                                    <td id="cosmeter">>
                                        <input type="text" class="inp" value="" name="newacmetered[cos]" 
                                    </td>
                                    <td><input type="text" class="inp" value="" name="newacmetered[name]"></td>
                                    
                                    <td>
                                        <a onclick="return addMeteredVar();"  class="prices menuitm " href="#"><span class="addsth">Add</span></a>
                                    </td>
                                </tr>
                            </tbody></table>
                    </td>
        ...
        <input type="hidden"  value="" name="newacmetered[rm]" id="rmvar">
        {literal}
            <script>
                function addMeteredVar(vr) {
                    $('#rmvar').before('<input type="hidden" name="make" value="newacmetered"/>');
                ...
                $(function() {
                    $('.newmeter').on('click', function() {
                        var self = $(this);
                        $(document).ajaxStart(function() {
                            self.addClass('disabled').off('click');
                        }).ajaxStop(function() {
                            self.parents('tr:eq(0)').hide();
                            if (!$('#cosmeter').find('select').length)
                                $('#cosmeter').html($('#cos').clone().attr({id: '', name: 'newacmetered[cos]'}));
                            $('#metered_var_editor').show();
