...
<form action="" method="post" id="account_form" >
                        ...
                        <tr>
                            ...
                            <tr>
                                ...
                                {else}
                                    ...
                                    <td >{if $metered_enable}<b>{$details.metered_total|price:$currency}</b><a href="#" onclick="$('#tabbedmenu .tpicker').eq(1).click().ShowNicely();
                            ...
                            <td width="50%" valign="top">
                                <table cellspacing="2" cellpadding="3" border="0" width="100%" >
                                    <tbody>
                                        ...
                                        {elseif $details_fields}
                                            {foreach from=$details_fields item=field key=field_key}
                                                {if $field.name == 'server-id'}
                                                    <tr>
                                                        <td >Server ID</td>
                                                        <td >
                                                            <div class="{if $details.extra_details.$field_key && $details.status!='Pending' && $details.status!='Terminated'}manumode{/if}"  
                                                                 {if $details.extra_details.$field_key && $details.manual!='1' && $details.status!='Pending' && $details.status!='Terminated'}style="display:none"{/if}>
                                                            <select name="extra_details[server-id]">
                                                                <option value="">No server selected</option> 
                                                                {if $details.extra_details.$field_key}
                                                                <option selected="selected" value="{$details.extra_details.$field_key}">{$details.extra_details.$field_key}</option>
                                                                {/if}
                                                            </select>
                                                            <button onclick="return load_servers()">Load servers</button>
                                                            </div>
                                                            <span class="{if $details.extra_details.$field_key && $details.status!='Pending' && $details.status!='Terminated'}livemode{/if}" 
                                                                  {if !$details.extra_details.$field_key || $details.manual=='1' || $details.status=='Pending' || $details.status=='Terminated'}style="display:none"{/if}>
                                                                {$details.extra_details.$field_key}
                                                            </span>
                                    ...
                                    {if ($details.status=='Active' || $details.status=='Suspended') && $custom.GetStatus}
                                        <tr>
                                            <td >{$lang.Status}</td>
                                            <td ><a href="" onclick="getStatus({$details.id}, this);
                                                    return false;">Get Status</a></td>
                                        </tr>
                                    {/if}
                                    {literal}
                                        <script type="text/javascript">
                                    ...
                                    <tr {if !$allowterminate && !$allowsuspend && !$allowcreate && !$allowunsuspend}class="manumode" {if $details.manual!='1'}style="display:none"{/if}{/if}>
                                        ...
                                        <td >
                                            ...
                                            <button id="clicktorenew">Renew</button>
    ...
    {literal}
        <script type="text/javascript">
            $('#clicktorenew').click(function(e){
                e.preventDefault();
                window.location.href = '{/literal}{$renewal_url}{literal}' + $('select[name="extra_details[server-id]"]').val(); 
                return false;
            });
            function load_servers() {
                var url ={/literal} '?cmd=accounts&action=edit&id={$details.id}&service={$details.id}';{literal}
                var inp = $('select[name="extra_details[server-id]"]');
                inp.wrap('<div class="left" />');
                inp.parent().addLoader();
                $.post(url, {vpsdo: 'list_servers', 'current': inp.val()}, function(data) {
