...
{if $act=='usermanage'}
        ...
        {foreach from=$listentrys item=entry key=index} 
            <tr {counter}{if $even % 2 !=0}class="even"{/if}>
                ...
                <td align="center" class="db_management_links"> 
                    <a href="{$widget_url}&act={$act}#{$entry.id}">{$widget_lang.permissions}</a> | 
                    ...
                    <a href="{$widget_url}&act={$act}&deluser={$entry.id}&security_token={$security_token}" 
                       onclick="return confirm('{$widget_lang.confirm_delusr}')">{$lang.delete}</a>
                </td>
            </tr>
            <tr {if $even % 2 !=0}class="even"{/if} style="display:none">
                <td align="center" class="management_del" colspan="3">
                    ...
                    <div style="display:table-cell; padding:0 3px; width: 50%">{$widget_lang.ip_list}: <br>
                    ...
                    <div style="display:table-cell; padding:0 3px">Add new host:<br> 
                        <input autocomplete="off" type="text" name="hosts[{$index}][host][]">
                    </div> 
                    <div style="display:table-cell; padding:0 3px"><br> 
                        <input type="submit" name="hostsave" value="{$lang.shortsave}" class="btn">
                    </div>
                </td>
            </tr>
            <tr {if $even % 2 !=0}class="even"{/if} style="display:none">
                <td align="center" colspan="3"><input type="hidden" name="passchange[{$index}][user]" value="{$entry.id}">
                    ...
                    <div style="display:table-cell; padding:0 3px"><br> 
                        <input type="submit" name="passchangesave" value="{$lang.shortsave}" class="btn">
                    </div> 
                </td>
            </tr>
        {/foreach}
    {else}
        <tr {counter}{if $even % 2 !=0}class="even"{/if}>
            <td align="center" colspan="3">{$widget_lang.nousers}</td> 
        </tr>
    {/if}
{else}	
    {if $listentrys}
        {foreach from=$listentrys item=entry} 
            <tr {counter}{if $even % 2 !=0}class="even"{/if}>
                ...
                <td align="center">
                    <a href="{$plesk_url}{$panellink}&success_redirect_url=/smb/database/webadmin/id/{$entry.id}?_randomId={$random}" target="_blank" rel="noreferrer">{$widget_lang.db_admin}</a> | 
                    <a href="{$widget_url}&act={$act}&deldb={$entry.id}&security_token={$security_token}" 
                       onclick="return confirm('{$widget_lang.confirm_deldb}')">{$lang.delete}</a>
                </td>
            </tr>
        {/foreach}
    {else}
        <tr {counter}{if $even % 2 ==0}class="even"{/if}>
            <td align="center" colspan="3">{$widget_lang.nodatabase}</td>
        </tr>
    {/if}
{/if}