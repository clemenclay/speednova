...
<a style="width:14px;"  class="menuitm menuf" href="?cmd=bugtracker&action=bandelete&id={$bug.id}&security_token={$security_token}" onclick="return confirm('Are you sure you want to delete this bug, and BAN author?');"><span class="rmsth">ban</span></a><!--
--><a onclick="return confirm('Are you sure you want to delete this bug?');" title="delete" class="menuitm menul" href="?cmd=bugtracker&action=bugdelete&id={$bug.id}&security_token={$security_token}&category_id={$bug.category_id}"><span class="delsth"></span></a>
</div></td>
...
<td width="150" style="background:#F0F0F3;color:#767679;font-size:11px" valign="top">{if $bug.firstname}Reporter: <b><a href="?cmd=clients&action=show&id={$bug.client_id}" target="_blank">{$bug.firstname} {$bug.lastname}</a></b>{/if}</td>
</tr>
</tbody></table></div></li>  
{foreachelse}
    <li style="background:#ffffff" class="power_row"><div style="border-bottom:solid 1px #ddd;">
