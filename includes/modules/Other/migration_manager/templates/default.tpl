    ...
    <div class="list-1">
        <ul>
            ...
            <li class="{if $action=='scenarios' || !$action || $action=='default'}active{/if}">
                <a href="?cmd=migration_manager&action=scenarios"><span>Migration scenarios</span></a>
            </li>

            <li class="{if $action=='cases'}active{/if} last">
                <a href="?cmd=migration_manager&action=cases"><span>Migration cases</span>{if $pending_builds>0}<span class="badge">{$pending_builds}</span>{/if}</a>
            </li>
               ...
               {if $action=='cases'}
				...
				<div class="subm1 haveitems">
					...
					<li {if !$finished}class="picked"{/if} >
						<a href="?cmd=migration_manager&action=cases" ><span >Pending/In-Progress Migration Cases</span></a>
                                        </li>
					<li {if $finished}class="picked"{/if}>
						<a href="?cmd=migration_manager&action=cases&finished=true"><span >Finished Migration Cases</span></a>
                                        </li>
					</ul>
					<div class="clear"></div>
				</div>
