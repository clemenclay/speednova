        ...
        <table class="clear" width="100%" cellspacing="0" cellpadding="0" style="margin-top:10px">
            <tbody>
                <tr>
                    ...
                    <th >{if $sortid}<a href="?cmd=module&module={$moduleid}&action=vlan_details&group={$sortid}&orderby=name|ASC" class="sortorder">{/if}Name{if $sortid}</a>{/if}</th>
                    <th style="min-width:100px">{if $sortid}<a href="?cmd=module&module={$moduleid}&action=vlan_details&group={$sortid}&orderby=descripton|ASC" class="sortorder">{/if}Description{if $sortid}</a>{/if}</th>
                    <th >Subnets</th>
                    <th width="115px" >{if $sortid}<a href="?cmd=module&module={$moduleid}&action=vlan_details&group={$sortid}&orderby=lastupdate|ASC" class="sortorder">{/if}Last update{if $sortid}</a>{/if}</th>
                    <th width="85px" >{if $sortid}<a href="?cmd=module&module={$moduleid}&action=vlan_details&group={$sortid}&orderby=changedby|ASC" class="sortorder">{/if}Changed by{if $sortid}</a>{/if}</th>
