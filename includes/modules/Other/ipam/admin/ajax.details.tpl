                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="ipam-linebrs">
                                <b>{if $group.client_id}<a href="?cmd=clients&action=show&id={$group.client_id}" target="_blank">#{$group.client_id} {$group.client}</a>{else}<em>None</em>{/if}</b><br/>
                        ...
                        <tr>
                            ...
                            <td colspan="5"> 
                                <div style="padding:10px 0px">
                                    {if !$sub.id}<a class="new_control" onclick="return add();" href="#">
                    ...
                    <table width="100%" cellspacing="0" cellpadding="5" border="0">
                        <tbody>
                            <tr>
                                <td width="80" valign="top">
                                    <div style="padding:10px 0px;">
                                        <a onclick="return editlist('{$sub.id}')" href="#" class="menuitm menu-auto" title="Edit" >
                                            <span class="editsth"></span>
                                        </a>
                                ...
                                <td class="ipam-linebrs">
                                    <b>{if $sub.client_id}<a href="?cmd=clients&action=show&id={$sub.client_id}" target="_blank">#{$sub.client_id} {$sub.client}</a>{else}<em>None</em>{/if}</b><br/>
    ...
    <form action="" method="post" onsubmit="return submitForm(this)" id="ipform" >
        ...
        <table class="clear" width="100%" cellspacing="0" cellpadding="0" style="margin-top:10px">
            <tbody>
                <tr>
                    ...
                    <th >{if $sortid}<a href="?cmd=module&module={$moduleid}&action=details&group={$sortid}&orderby=wanip|ASC" class="sortorder">{/if}WAN IP</a></th>
        ...
        {foreach from=$list item=line}
            <tr>
                ...
                <td ><div>{$line.wanip}</div><span class="editbtn" onclick="edit(this,{$line.server_id})">Edit</span></td>
                ...
                <td class="fs11 ownercol"><div class="left">{if $line.client_id!='0'}Client: <a href="?cmd=clients&action=show&id={$line.client_id}" target="_blank">#{$line.client_id}</a>{else}-{/if}{if $line.account_id}
                    ...
                    <td style="padding:5px 0 0 10px; width:40px">
                        <a class="editsth" title="Edit" href="#" onclick="return advEdit('{$line.server_id}', '{$line.id}')" ></a>
                ...
                {$lang.showing} 
                <span id="sorterlow">{$sorterlow}</span> - <span id="sorterhigh">{$sorterhigh}</span> {$lang.of} <span id="sorterrecords">{$sorterrecords}</span>
