...
{if !$mode}{literal}
        <script type="text/javascript">
            ...
            function inichosen() {
                ...
                $('#cidr_ipv4, #cidr_ipv6').change(function(){
                    $('#ro-cidr').val($(this).val())
                })
            }
            inichosen();
        </script>
            ...
            <td class="fright">
                ...
                <div class="form_container">
                    <form action="?cmd=module&module={$moduleid}" method="post" onsubmit="return false">
                        ...
                        <select id="ipvtype" name="type" class="w250" onchange="vtype($(this).val())">
                        ...
                        <select  class="w250" name="vlan_id" default="{$group.vlan_id}" id="vlan_id" load="?cmd=ipam&action=vlan" >
                            <option value="0" {if !$group.vlan_id || $group.vlan_id=='0'}selected="selected"{/if}>None</option>
                        ...
                        <label>Auto-assign <a class="vtip_description" title="Enable this option if you want HostBill to use this pool with auto-provisioned servers"></a></label>
                        ...
                        <div id="pool_settings" style="display:none">
                            ...
                            <label>Gateway</label>
                            <input type="text" name="gateway" value=""  class="w250"/>

                            <div class="clear"></div>

                            <label>Fill WAN IP </label>
                            <input type="checkbox" name="fillwanips" value="1" onclick="wan(this)"
                                   style="margin: 6px 20px 20px;"/>
                            <div class="clear"></div>
                            
                            <div class="wanip" style="display:none">
                                <label>WAN Network (CIDR)</label>
                                <input type="text" name="wanip" value=""  class="w250 left" style="width:200px;margin-right:10px;"/>
                                <input id="ro-cidr" class="w250 left" style="width:80px" readonly />
                            </div>
                        </div>
                            ...
                            <form action="?cmd=module&module={$moduleid}" method="post" onsubmit="return false" >
                                ...
                                {if $forms[$api.id] && is_array($forms[$api.id])}
                                    {foreach from=$forms[$api.id] item=field}
                                        <label>{$field.name}
                                            {if $field.descr}<br />
                                                <small>{$field.descr}</small>
                                            {/if}
                                        </label>
                                        <{$field.type} {if $field.attr}{foreach from=$field.attr item=attr key=atn}{$atn}="{$attr}"{/foreach}{/if}>
                                    {/foreach}
                                {else}
