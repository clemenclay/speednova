            ...
            <table width="100%" cellspacing="0" cellpadding="5" border="0">
                <tbody><tr>
                        <td width="120" valign="top"><div style="padding:10px 0px;">
                                <a  class="sorter-ha menuitm menu-auto" href="?cmd=module&module={$moduleid}&do=floors&colo_id={$colo.id}"><span >Floors</span></a>
                                <a  title="Edit" class="menuitm menu-auto" href="?cmd=module&module={$moduleid}&do=floors&colo_id={$colo.id}" 
                                ...
                                <a  onclick="return confirm('Are you sure you want to delete this colocation?');" title="delete" 
                                    class="menuitm menu-auto" href="?cmd=module&module={$moduleid}&do=removecolo&colo_id={$colo.id}">
                                    <span class="delsth"></span>
                                </a>
                            </div></td>
                        <td  style="line-height:20px">
                            <h3><a href="?cmd=module&module={$moduleid}&do=floors&colo_id={$colo.id}">{$colo.name}</a></h3>
                            Floors: <a href="?cmd=module&module={$moduleid}&do=floors&colo_id={$colo.id}"><strong>{$colo.floors}</strong></a><br />
                            Racks: <strong>{$colo.racks}</strong> {foreach from=$colo.rack_list item=rack}<em class="fs11" style="padding-left:20px"><a href="?cmd=module&module={$moduleid}&do=rack&rack_id={$rack.id}">{$rack.name} ({$rack.units} U)</a></em>{/foreach} <em class="fs11" style="padding-left:20px"><a href="?cmd=module&module={$moduleid}&do=floors&colo_id={$colo.id}">...</a></em><br />
