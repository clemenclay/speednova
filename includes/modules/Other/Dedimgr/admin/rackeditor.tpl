...
<p id="rackview_switch">
    <a class="menuitm menu-auto {if !$rackview}activated{/if}" href="?cmd=module&module={$moduleid}&do=listview&rel=rack&id={$rack.id}" data-rel="rack_view">Details</a>
    <a class="menuitm menu-auto {if $rackview=='list'}activated{/if}" href="?cmd=module&module={$moduleid}&do=listview&rel=rack&id={$rack.id}&type=list" data-rel="rack_list">List</a>
    <a class="menuitm menu-auto {if $rackview=='ipam'}activated{/if}" href="?cmd=module&module={$moduleid}&do=listview&rel=rack&id={$rack.id}&type=ipam" data-rel="rack_ipam">IPAM</a>
</p>
    ...
    {foreach from=$rackips item=i}
        <tr class="rack-list-item">
            ...
            <td class="rack-list-act">
                <div>
                    <a onclick="return editRackItem('{$i.item_id}')" href="#" class="menuitm menu" title="Edit" ><span class="editsth"></span></a></a>
                </div>
            </td>
        </tr>
    {/foreach}
        ...
        {foreach from=$positions item=i}
            {if $i}
                <tr class="rack-list-item">
                    ...
                    <td class="rack-list-act">
                        <div>
                            <a onclick="return editRackItem('{$i.id}')" href="#" class="menuitm menu-auto" title="Edit"><span class="editsth"></span></a>
                            <a class="menuitm menu-auto" title="delete" onclick="return confirm('Are you sure you want to remove this item?');" 
...
<table border="0" cellpadding="0" cellspacing="0" width="100%" id="rack_view">
    ...
    <tbody >{foreach from=$rack.positions.Zero item=i key=k}
                        ...
                        {else}
                            <tr data-position="{$k}" class="dragdrop" data-units="1">
                                <td class="rack_1u canadd">
                                    <a class="newitem" href="#" onclick="return addRItem('1','Zero')">Add new item</a>
                                </td>
                            </tr>
                        {/if}
                    {/foreach}
