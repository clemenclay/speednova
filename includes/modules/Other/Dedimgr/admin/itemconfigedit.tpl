<h1> 
    <strong><a href="?cmd=module&module={$moduleid}&do=floors&colo_id={$rack.colo_id}">Colocation: {$rack.coloname}</a></strong>
    &raquo; <strong><a href="?cmd=module&module={$moduleid}&do=floor&floor_id={$rack.floor_id}">Floor: {$rack.floorname}</a> <em>{if $rack.room} Room: {$rack.room}{/if}</em></strong>
    &raquo; <strong><a href="?cmd=module&module={$moduleid}&do=rack&rack_id={$rack.id}">Rack: {$rack.name}</a> </strong> 
...
<form action="?cmd=module&module={$moduleid}&action=itemconfigedit&item_id={$item.id}&id={if $entry.id}{$entry.id}{else}new{/if}" method="POST">
    <table style="width: 95%" cellpadding="6" cellspacing="0">
        ...
        <tr>
            <td colspan="2">
                <input type="submit" value="{if $upmode}Update configuration{else}Create configuration{/if}" name="save" \>
            </td>
        </tr>
    </table>
</form>