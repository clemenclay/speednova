...
<a onclick="return unassignRackItem('{$itm.id}')" title="delete" class="menuitm" href="#"><span class="rmsth">Unassign</span></a>
</div></td>
<td>
    Colo: <a href="?cmd=module&module=dedimgr&do=floors&colo_id={$itm.colo_id}" target="_blank" class="external">{$itm.colo_name}</a>,
    Floor: <a href="?cmd=module&module=dedimgr&do=floor&floor_id={$itm.floor_id}" target="_blank" class="external">{$itm.floor_name} {if $itm.room}(Room: {$itm.room}){/if}</a>,
    Rack: <a href="?cmd=module&module=dedimgr&do=rack&rack_id={$itm.rack_id}" target="_blank" class="external">{$itm.rack_name}</a>,
    Item: <a href="?cmd=module&module=dedimgr&do=rack&rack_id={$itm.rack_id}&expand={$itm.id}" target="_blank" class="external"><b>#{$itm.id}: {$itm.name} - {$itm.label}</b></a> <em class="fs11">position: {$itm.position}</em>
