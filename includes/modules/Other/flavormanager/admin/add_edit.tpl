    ...
    <div class="list-1">
        <ul>
            ...
            <li class="{if $action == 'ip'}active{/if}">
                <a {if $action != 'storage'} href="?cmd={$modulename}&action=ip"{/if}><span>IP Flavor List</span></a>
            </li>

            <li class="{if $action == 'add' || $action == 'edit'}active{/if} last">
                <a {if $action != 'add' && $action == 'edit'}href="?cmd={$modulename}&action=add"{/if}>
                    <span>
                        {if $action == 'edit'}{$entry.name|truncate} - {if $entry.type == 0}VM Flavor{elseif $entry.type==1}Storage Flavor{else}IP Flavor{/if}
                        {else}New Flavor
                        {/if}
                    </span>
                </a>
        ...
        <div class="list-2">
            <div class="subm1 haveitems">
                <ul id="typepick">
                    ...
                    <li ><a href="#IP"><span>IP Flavor</span></a></li>
                </ul>
            </div>
        </div>
    {/if}
...
<form method="post" action="?cmd={$modulename}&action={$action}" id="status_plugin"  style="padding: 10px 0;">
    ...
    <table cellpadding="5" style="width: 100%">
        ...
        <tr>
            <td style="width: 140px; text-align: right">
                <label>
                    ...
                    <b class="tabs IP">Price for 1IP/hour</b>
                </label>
        ...
        <input type="hidden" id="typepicked" name="type" value="{if $entry.type == 0}VM{elseif $entry.type==1}STORAGE{else}IP{/if}"/>
...
{/if}
        ...
        {if $action!="edit" || $entry.type == 2 }
            <tbody class="tabs IP">
            {foreach from=$variables.IP item=mods key=key}
                <tr class="modframe {foreach from=$mods item=n key=mid}mod_{$mid} {/foreach}">
                    <td style="width: 140px; text-align: right">
                        {foreach from=$mods item=data key=mid}
                            <label class="modframe mod_{$mid}">{$data.name}</label>
