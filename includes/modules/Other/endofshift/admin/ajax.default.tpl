...
Tickets left note in ({$summary.totals.notes})
{foreach from=$summary.notes item=su}
{$system_url}admin/?cmd=tickets&action=view&list=all&num={$su}
{/foreach}

{if $summary.totals.replied==0 && $summary.totals.closed==0 && $summary.totals.unread==0 && $summary.totals.notes==0 }
