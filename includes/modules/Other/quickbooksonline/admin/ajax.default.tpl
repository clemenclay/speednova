...
{foreach from=$list item=entry}
    <tr class="havecontrols">
        <td><input type="checkbox" name="selected[]" value="{$entry.id}"/></td>
            {if $action == 'invoices'}
            <td>
            ...
            <td><span class="{$entry.status}">{if $lang[$entry.status]}{$lang[$entry.status]}{else}{$entry.status}{/if}</span></td>
        ...
        {/if}
        <td><a href="?cmd={$modulename}&action={$action}&selected={$entry.id}&security_token={$security_token}">Export</a></td>
    </tr>
{foreachelse}
    <tr>
