...
{foreach from=$cf.items item=cit}
    <input name="{if $cf_opt_name && $cf_opt_name != ''}{$cf_opt_name}{else}custom{/if}[{$cf.id}][{$cit.id}]" 
           ...
           class="styled custom_field custom_field_{$cf.id}" type="text" size="2"
           onkeyup="$(document).trigger('hbcart.changeform', [this]);if(typeof(simulateCart) == 'function')simulateCart('{if $cf_opt_formId && $cf_opt_formId != ''}{$cf_opt_formId}{else}#cart3{/if}');"/> {if $cit.name || $cit.price}x{/if} {if $cit.name!='None'}{$cit.name}{/if}
    {if $cit.price!=0}(
