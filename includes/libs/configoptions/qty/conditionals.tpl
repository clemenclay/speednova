...
<li style="background:#ffffff" id="conditional_{$k}">
    ...
    <input type="hidden" name="config[conditional][{$k}][targetval]"  value="{$f.targetval}"/>
        ...
        <table width="100%" cellspacing="0" cellpadding="5" border="0">
            <tbody><tr>
                    <td >
                        If <b>{$field.name}</b> {if $f.condition=='more'}is more than{elseif $f.condition=='less'}is less than{else}equals to{/if}
                        {$f.conditionval},
                       than
        ...
        {if $f.targetval} to {$f.targetval} {/if}
                    ...
                    <td valign="top" align="right">
                        <div style="padding:5px 0px;"><a href="#"  style="width:12px;height:14px;display:block; box-sizing: content-box;" class="menuitm" title="delete" onclick="$('#condition_field_new_val').val(0);$('#conditional_{$k}').remove();saveChangesField(); return false"><span class="delsth"></span></a>
                        </div>
                    </td>
                </tr>
            </tbody>
