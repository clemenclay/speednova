{foreach from=$cf.items item=cit name=nit}
    ...
    <input
        ...
        {if $contents[1][$cf.id][$cit.id]}checked="checked"{/if} onclick="if (typeof (simulateCart) == 'function')
                    simulateCart('{if $cf_opt_formId && $cf_opt_formId != ''}{$cf_opt_formId}{else}#cart3{/if}')" class="pconfig_ styled"/> 
