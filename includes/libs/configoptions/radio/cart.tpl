...
<input name="{if $cf_opt_name && $cf_opt_name != ''}{$cf_opt_name}{else}custom{/if}[{$cf.id}]"  
       value="{$cit.id}" {if $smarty.foreach.its.first}id="custom_field_{$cf.id}"{/if} type="radio"  {if $contents[1][$cf.id][$cit.id]}checked="checked"{/if} 
       ...
       class="pconfig_ styled custom_field_{$cf.id}"/> 
