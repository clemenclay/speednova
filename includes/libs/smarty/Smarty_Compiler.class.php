...
//                                       , $source_content);
        ...
        $left_bracket = $this->_quote_replace($this->left_delimiter);
        $right_bracket = $this->_quote_replace($this->right_delimiter);
        $source_content = preg_replace_callback($search,
            function($matches) use ($left_bracket,$right_bracket) {
                return ""
                . $left_bracket
                . 'php'
    ...
    function _expand_quoted_text($var_expr)
    {
        ...
        // escaping char left before variable would result in syntax error
        foreach($_replace as $find){
            $pos = strpos($_return, $find);
            if($pos && substr($_return, $pos-1, 1) == '\\' && substr($_return, $pos-2, 2) != '\\\\'){
                $_return =  substr($_return, 0, $pos).'\\'. substr($_return, $pos);
            }
        }
