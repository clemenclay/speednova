...
<div style="width:630px" class="left">
<div class="wbox">
		...
		<table width="100%" class="checker table" cellpadding="0" cellspacing="0">
			...
			{foreach from=$license item=data key=key name=alter}
                ...
                <tr {if $alter%2==0}class="even"{/if}>
                    <td {if $smarty.foreach.alter.laast}style="border:none" {/if}align="right" width="160">{if $lang[$key]}{$lang[$key]}{else}{$key}{/if}:</td>
                    <td {if $smarty.foreach.alter.laast}style="border:none" {/if}>{$data}</td>
                </tr>
                {counter name=alter}
				{/if}
			{/foreach}
