<table class="data-table backups-list"  width="100%" cellspacing=0>
    ...
    {foreach item=schedule from=$schedules}
        <tr>
            <td align="right">
                {$schedule._duration}
                {if $schedule._period=='weeks'}
                    {$lang.yweek}
                {elseif $schedule._period=='days'}
                    {$lang.yday}
                {elseif $schedule._period=='months'}
                    {$lang.ymonth}
                {elseif $schedule._period=='years'}
                    {$lang.yyear}
                {/if}
            ...
            <td>
                ...
                <a title="{$lang.delete}" href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=backupschedule&diskid={$diskid}&vpsid={$vpsid}&schedule_id={$schedule._id}&make=deleteschedule&security_token={$security_token}" onclick="return confirm('{$lang.suretodeleteschedule}');" class="small_control small_delete fs11" >
                    {$lang.delete}
                </a>
        ...
        <table class="data-table backups-list"  cellspacing="0" style="border-top:solid 1px #DDDDDD;">
            ...
            <tr>
                <td>
                    ...
                    <input name="duration" value="1" size="2" class="styled"/> 
                </td>
                <td>
                    <br/>
                    <select name="period">
