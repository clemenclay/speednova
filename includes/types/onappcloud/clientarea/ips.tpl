...
<form action="" method="post">
    ...
    <table cellspacing="0" cellpadding="0" border="0" width="100%" class="checker">
        <tbody><tr>
                <td colspan="2">
                    <div>
                        <span class="slabel">{$lang.interface} <a class="vtip_description" title="{$lang.selectwhichnetworkinterfacethisaddr}"></a></span>
            ...
            <tr>
                <td colspan="2">
                    <div>
                        <span class="slabel">{$lang.ipadd}</span>
                        <select style="min-width:250px;" name="assign[ip_address_id]" id="ip_address_id">
                            <option value="">{$lang.selectinterfacefirst}</option>
                        </select> <img src="includes/types/onappcloud/images/ajax-loader.gif" style="display:none" id="iploader" alt="{$lang.loading}"/>
...
<script type="text/javascript">
    function loadIps() {
        ...
        $('#iploader').hide();
            ...
            $('#ip_address_id').append('<option value="">{/literal}{$lang.selectinterfacefirst}{literal}</option>');
            return;
        }
        $.getJSON('{/literal}?cmd=clientarea&action=services&service={$service.id}&vpsdo=ips&vpsid={$vpsid}&do=fetchips{literal}', {network_interface_id:w}, function(data){
            $('#ip_address_id').removeAttr('disabled');
            $('#iploader').hide();
            if(data && data.ips && data.ips instanceof Array) {
...
<table class="data-table backups-list"  width="100%" cellspacing=0>
    ...
    <tr>
        ...
        <td>{$ip.interface}</td>
        <td>
           {if $o_sections.o_deleteip} 
               <a title="{$lang.delete}"  href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=ips&vpsid={$vpsid}&do=deleteip&ipid={$ip.id}&security_token={$security_token}" onclick="return  confirm('{$lang.suretodeleteip}?')" class="small_control small_delete fs11">{$lang.delete}</a>
           {/if}
        </td>
    </tr>
    {foreachelse}
...
<div style="padding:10px 0px;text-align:right">
    ...
    {/if}
    <input type="button" class="blue" onclick="return bindonapplogin()" value="{$lang.rebuildnetwork}"/>

    {if $o_sections.o_addip && $allowaddingip}
    ...
    <span class="text">{$lang.freeipleft1} <b>{$allowaddingip[1]}</b> {$lang.freeipleft} 
        <i style="font-size:11px">{if $allowaddingip[1] || $allowaddingip[2]}( {if $allowaddingip[1]}{$lang.public}: {$allowaddingip[1]} {/if} 
        {if $allowaddingip[2]}{$lang.private}: {$allowaddingip[2]} {/if} ){/if}</i>
...
<div id="cloginform" style="display:none">
  <form action="?cmd=clientarea&action=services&service={$service.id}&vpsdo=ips&vpsid={$vpsid}&do=rebuildnetwork&security_token={$security_token}" method="post" >  <table width="100%" cellspacing="0" cellpadding="0" border="0" class="checker">

        <tr>
            <td colspan="2"><span class="slabel">{$lang.forcereboot}</span> <span class="slabel"><input type="checkbox" value="1" name="force" /></span></td>
        </tr>
        <tr>
            <td colspan="2"><span class="slabel">{$lang.requiredstartup}</span> <span class="slabel"><input type="checkbox" value="1" name="startup" checked="checked"/></span></td>
        </tr>
        <tr class="lastone">
            <td align="center" style="padding:10px 0px;">
                {$lang.rebuildconfirm}
                    <input type="submit" value="{$lang.rebuildnetwork}" class="blue btn btn-success" />
