<div id="lock" {if $VMDetails.locked!='false'}style="display:block"{/if}>
    ...
    <img src="includes/types/onappcloud/images/ajax-loader.gif" alt="">
    {$lang.server_performing_task} <a href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=vmdetails&vpsid={$vpsid}">
        <b>
            {$lang.refresh}</b>
    </a>
...
<ul id="vm-menu" >
    {if $VMDetails.power=='true'}
        ...
        {if $o_sections.o_reboot}<li>
                <a href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=reboot&vpsid={$vpsid}&security_token={$security_token}" onclick="return confirm('{$lang.sure_to_reboot}?');">
                    <img alt="{$lang.reboot}" src="includes/types/onappcloud/images/icons/24_arrow-circle.png">
                    <br>
                    {$lang.reboot}</a>
            </li>
        {/if}
    ...
    {if $o_sections.o_rebuild}<li>
            <a href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=reinstall&vpsid={$vpsid}&security_token={$security_token}" >
                <img alt="{$lang.rebuild}" src="includes/types/onappcloud/images/icons/24_blueprint.png">
                <br>
                {$lang.rebuild}
            </a>
        </li>
    {/if}
    {if $VMDetails.power=='true'}
        {if $o_sections.o_startstop}
            <li>
                <a href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=forcepoweroff&vpsid={$vpsid}&security_token={$security_token}" onclick="return confirm('{$lang.sure_to_poweroff}');">
                    <img alt="{$lang.ForcePowerOff}" src="{$template_dir}../common/cloudhosting/images/icons/poweroff.png">
                    <br>
                    {$lang.forceoff}
                </a>
            </li>
        ...
        {if $o_sections.o_reboot}
            <li>
                <a href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=rebootrecovery&vpsid={$vpsid}&security_token={$security_token}" onclick="return confirm('{$lang.reboot_recovery}');">
                    <img alt="{$lang.recovery}" src="includes/types/onappcloud/images/icons/24_lifebuoy.png">
                    <br>
                    {$lang.recovery}
                </a>
            </li>
    ...
    {else}
        {if $o_sections.o_startstop}<li>
                <a href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=poweronrecovery&vpsid={$vpsid}&security_token={$security_token}" onclick="return confirm('{$lang.start_recovery}');">
                    <img alt="{$lang.recovery}" src="includes/types/onappcloud/images/icons/24_lifebuoy.png">
                    <br>
                    {$lang.recovery}
                </a>
    ...
    {if $o_sections.o_logs}<li>
            <a href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=logs&vpsid={$vpsid}&security_token={$security_token}">
                <img alt="{$lang.logs}" src="includes/types/onappcloud/images/icons/receipt-text.png">
                <br>
                {$lang.logs}
            </a>
        </li>
    ...
    {if $o_sections.o_scale}
        <li>
            <a href="?cmd=clientarea&action=services&service={$service.id}&vpsid={$vpsid}&vpsdo={if $provisioning_type!='single'}editvm{else}upgrade{/if}">
                <img alt="{$lang.scale}" src="includes/types/onappcloud/images/icons/24_equalizer.png">
                <br>
    ...
    {/if}
    {if $o_sections.o_resize} {*flavor*}
        <li>
            <a href="?cmd=clientarea&action=services&service={$service.id}&vpsid={$vpsid}&vpsdo=resize">
                <img alt="{$lang.scale}" src="includes/types/onappcloud/images/icons/24_equalizer.png">
                <br>
                Resize
            </a>
    ...
    {/if}
        ...
        <a {if $provisioning_type=='single'}href="{$ca_url}clientarea&action=services&service={$service.id}&cid={$service.category_id}&cancel"{else} href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=destroy&vpsid={$vpsid}&security_token={$security_token}" onclick="return  confirm('{$lang.sure_to_destroy}?')" {/if}>
            <img alt="{$lang.delete}" src="includes/types/onappcloud/images/icons/24_cross.png">
            <br>
            {if $provisioning_type=='single'}{$lang.cancelvps}
            {else}{$lang.delete}
            {/if}
        </a>
