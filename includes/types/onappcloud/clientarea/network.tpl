...
<ul class="sub-ul">
    {if $o_sections.o_rdns}<li ><a  href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=ips&vpsid={$vpsid}&widget=o_rdns" class="{if $vpsdo=='firewall'}active{/if}"><span>{$lang.reversedns}</span></a></li>{/if}
...
<div class="content-bar {if $subdo=='addinterface' || $subdo=='edit' || $subdo=='assignip'}nopadding{/if} section-network section-network-{$vpsdo}">
