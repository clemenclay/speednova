...
<table class="data-table backups-list"  width="100%" cellspacing=0>
    ...
    <tr>
        ...
        {if $o_sections.o_backup_auto}  
        <td>
            {if $disk->_is_swap != "true" && $disk->_locked!="true" && !$vm.support_incremental_backups}
        ...
        <td>
            {if $disk->_is_swap != "true"}
                {if $disk->_locked=="true"}
                    {$lang.locked}
                {else}
                    {if $o_sections.o_editdisk}  
                        <a title="{$lang.edit}" href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=disks&do=editor&diskid={$disk->_id}&vpsid={$vpsid}" 
                           class="small_control small_pencil fs11" >
                            {$lang.edit}
                        </a> 
                    {/if}
                    {if $o_sections.o_backup && !$vm.support_incremental_backups}  
                    ...
                    {if $disk->_primary == "true"}
                    {elseif $disk->_is_swap == "true"}
                    {else} <br/><a title="{$lang.delete}" href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=disks&do=delete&diskid={$disk->_id}&vpsid={$vpsid}&security_token={$security_token}" 
                            onclick="return confirm('{$lang.suretoremovedisk1}');" class="small_control small_delete fs11" >
                            {$lang.delete}
                        </a>
                    {/if}
                {/if}
...
{if  $o_sections.o_backup && $vm.support_incremental_backups}
    <input type="button" class="blue" title="{$lang.backup}" 
           onclick="if(confirm('{$lang.suretocreatebkp|escape:'javascript'}')) window.location='?cmd=clientarea&action=services&service={$service.id}&vpsdo=backups&do=create&diskid={$disk->_id}&vpsid={$vpsid}&security_token={$security_token}'" 
