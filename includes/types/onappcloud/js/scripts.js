...
var init_sliders = function() {
    $(".slider").each(function(index, object) {
        ...
        var divide_total = divide.length ? parseInt(divide.attr("max").toString()) : 0;
        ...
        if (target.length)
        {
            ...
            var update_target = function() {
                ...
                if (divide.length && total) {
                    var divmax = total * divide_total;
                    var tot = parseInt(divmax / (parseInt(target.attr("value").toString()) || 0));
...
function filtertemplates(place, check) {
    if(check === null)
        check = true;
...
function swapcheck(id) {
    ...
    var recalc = true;
    var max = parseInt($('#disk-row .slider').slider("option", "max"));
    var total = parseInt($('#disk-row .slider').attr("total"));
    if (total - max > 1) {
        recalc = false;
    }
    ...
    licesetypecheck($('#virtual_machine_license_type').val());
}

var license_types = null;
var lic_lastchecked = null;
function licesecheck() {
    var osid = $('#virtual_machine_template_id').val(),
        type = $('#virtual_machine_license_type').val();
    if (!license_types) {
        license_types = $('#license_type option').clone();
    }
    
    if(lic_lastchecked == osid)
        return false;
    lic_lastchecked = osid;

    $('#license_type, #license_own').hide().find('select, input').attr('disabled', 'disabled').prop('disabled', true);
    $('#license_type option').remove();
    if (windows.length && windows[osid]) {
        $('#ospreloader').show();
        $.get(window.location.href.replace(/vpsid=[^&]*/,'').replace(/vpsdo=[^&]*/,'vpsdo=createvm') + '&make=osdetails&os=' + osid, function(data) {
            $('#ospreloader').hide();
            if (typeof data.license == 'object') {
                var activel = []
                $.each(data.license, function(key) {
                    if (this.valueOf() === true)
                        activel.push(key);
                })
                console.log(activel);
                if (activel.length == 1 && activel[0] == 'own') {
                    $('#virtual_machine_license_type').removeAttr('disabled').prop('disabled', false).val('own').append(license_types.filter('[value=own]')).change();
                } else if (activel.length > 1) {
                    $('#license_type').show().find('select').removeAttr('disabled').prop('disabled', false);
                    $('#virtual_machine_license_type').append(license_types.filter(function() {
                        return activel.indexOf($(this).val()) >= 0
                    })).val(type).change();
                }
            }
        })
...
function licesetypecheck(val){
    if(val && val=='own'){
        $('#license_own').show().find('input').removeAttr('disabled').prop('disabled',false);
    }else{
        $('#license_own').hide().find('input').attr('disabled', 'disabled').prop('disabled',true);
    }
}