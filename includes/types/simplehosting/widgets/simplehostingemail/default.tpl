			...
			{counter start=1 skip=1 assign=even}
				...
				{foreach from=$listentrys item=entry key=index} 
					<tr {counter}{if $even % 2 !=0}class="even"{/if}>
						...
						<a href="?cmd={$cmd}&action={$action}&service={$service.id}&widget={$widget.name}&act={$act}&deluser={$entry.user}&domain={$entry.domain}" onclick="return confirm('Do You really want to delete this Email account?')">{$lang.delete}</a> 
						</td>
