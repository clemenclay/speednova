            ...
            {foreach from=$items item=item}
                <table class="table table-condensed table-bordered table-striped" >
                    ...
                    <tr>
                        ...
                        <td colspan="2" class="wstate wstateon" item_id="{$item.item_id}" port_id="{$item.port_id}">
                            <span >Loading</span>
                            <form action="" method="post" onsubmit="return confirm('Are you sure?')" class="form-inline pull-right" style="margin:0px">
                                <strong>Power: </strong>
                                <input type="hidden" name="item_id" value="{$item.item_id}" />
                                <input type="hidden" name="port_id" value="{$item.port_id}" />
                    ...
                    <tr>
                        <td colspan="3"  class="port_extras"  item_id="{$item.item_id}" port_id="{$item.port_id}">
