								...
								<tr>
									...
									<td>
										<table  width="100%" cellpadding="0" cellspacing="0">
											<tr class="lastone"><td  width="150" align="right">{$lang.en_appremailpick}</td><td>
							...
							{if (!$service.organization && !$organization.updated) || $editOrg}
								<tbody>
									<tr>
										...
										<td>
											<table class="checker" width="100%" cellpadding="0" cellspacing="0">
												<tr class="lastone">
						...
						{if (!$service.contacts && !$contacts.updated) || $editContacts}
							<tbody>
								{foreach from=$contactType item=iname}
									<tr {if $iname == 'billing'}id="bcontact" style="display:none"{elseif $iname == 'tech'}id="tcontact" style="display:none"{/if}>
										<td align="right" valign="top">
											<strong>
												{if $iname == 'admin'}
										...
										<td>
											<table border="0" cellpadding="3" cellspacing="0" width="100%">
												...
												<tr class="lastone">
													...
													<td>
														<input name="{$iname}[Address1]"  class="styled" value="{if $contacts[$iname].Address1}{$contacts[$iname].Address1}{elseif $service.contacts[$iname].Address1}{$service.contacts[$iname].Address1}{else}{$clientdata.address1}{/if}" />
														/ <input name="{$iname}[Address2]"  value="{if $contacts[$iname].Address2}{$contacts[$iname].Address2}{elseif $service.contacts[$iname].Address2}{$service.contacts[$iname].Address2}{else}{$clientdata.address2}{/if}" class="styled"/>
													</td>
												</tr>
												<tr class="lastone">
												...
												<tr class="lastone">
													...
													<td>
														<input name="{$iname}[State]"  value="{if $contacts[$iname].State}{$contacts[$iname].State}{elseif $service.contacts[$iname].State}{$service.contacts[$iname].State}{else}{$clientdata.state}{/if}" class="styled"/>
														/ <input name="{$iname}[PostalCode]"  value="{if $contacts[$iname].PostalCode}{$contacts[$iname].PostalCode}{elseif $service.contacts[$iname].PostalCode}{$service.contacts[$iname].PostalCode}{else}{$clientdata.postcode}{/if}" class="styled" size="6"/>
											...
											<tr class="lastone">
												...
												<td>
													+ <input size="2" name="{$iname}[PreFix]" class="styled" value="{if $contacts[$iname].PreFix}{$contacts[$iname].PreFix}{elseif $service.contacts[$iname].PreFix}{$service.contacts[$iname].PreFix}{else}{/if}"/>.
													<input name="{$iname}[Phone]"  value="{if $contacts[$iname].Phone}{$contacts[$iname].Phone}{elseif $service.contacts[$iname].Phone}{$service.contacts[$iname].Phone}{else}{$clientdata.phonenumber}{/if}" class="styled"/>
												</td>
											</tr>
											<tr class="lastone">
												<td width="150" align="right">Fax</td>
												<td>
													+ <input size="2" name="{$iname}[FPreFix]" class="styled" value="{if $contacts[$iname].FPreFix}{$contacts[$iname].FPreFix}{elseif $service.contacts[$iname].FPreFix}{$service.contacts[$iname].FPreFix}{else}{/if}"/>.
													<input name="{$iname}[Fax]"  value="{if $contacts[$iname].Fax}{$contacts[$iname].Fax}{elseif $service.contacts[$iname].Fax}{$service.contacts[$iname].Fax}{else}{/if}" class="styled"/>
												</td>
											</tr>
										</table>
...
{if $service.status=='Active'}
	{if $status}
				...
				<div class="wbox_content">
					<table width="100%" cellspacing="0" cellpadding="0" border="0" class="checker table">
						<tr>
							...
							<td>{$lang.certificationperiods}:</td>
							<td>{$status.period} {$lang.years}</td>

						</tr>
						<tr class="even">
							<td>{$lang.webserver}:</td>
							<td>{$status.server_type}</td>
							<td>ID:</td>
							<td>{$status.order_id}</td>
						</tr>
						<tr>
							<td>{$lang.validationemail}:</td>
							<td>{$status.approver_email}</td>
							<td>{$lang.goto}:</td>
							<td><a href="http://{$status.domain}">{$status.domain}</a></td>
						</tr>
						<tr class="even">
							<td>{$lang.price}:</td>
							<td>{$service.total|price:$currency}</td>
							<td>{$lang.creationdate}:</td>
							<td>{if $status.start_date}{$status.start_date|dateformat:$date_format}{elseif $status.issue_date}{$status.issue_date|dateformat:$date_format}{else}{$service.date_created|dateformat:$date_format}{/if}</td>
 
						</tr>
						<tr>
							<td>{$lang.supplierorderid}:</td>				
							<td>{$status.supplier_order_id}</a></td>
							<td>{$lang.expirydate}:</td>				
							<td>{if $status.expiry_date}{$status.expiry_date|dateformat:$date_format}{else}{$service.cert_expires|dateformat:$date_format}{/if}</a></td>
						</tr>
						
							<tr class="lastone ">
								<td colspan="4" style="text-align: right">
									{if $status.sitelock_link}
										<input type="submit" value="Access SiteLock" name="submit" style="font-weight:bold" class="btn btn-success" onclick="$('#edo').val('sitelock')"/>
									{/if}
									{if $status.state=='awaiting-approval'}
									<input type="submit" value="{$lang.resendapproveemail}" name="submit" style="font-weight:bold" class="btn btn-primary" onclick="$('#edo').val('sendmail')"/>
									{/if}
									{if $status.state=='active' || $status.state=='compleated'}
									<input type="submit" value="{$lang.resendcertificateemail}" name="submit" style="font-weight:bold" class="btn btn-primary" onclick="$('#edo').val('resendcert')"/>
									{/if}
									{if $status.is_renewable=='1'}
									<input type="submit" value="{$lang.renew}" name="submit" style="font-weight:bold" class="btn btn-primary" onclick="$('#edo').val('renew')"/>
									{/if}
									<input type="submit" value="{$lang.synchronize}" name="submit" style="font-weight:bold" class="btn btn-success" onclick="$('#edo').val('Synchronize')"/>
									<input type="hidden" name="edo" value="" id="edo"/>
								</td>
							</tr>
		...
		{if $status.notes_list}
			<div class="wbox">
				<div class="wbox_header">
					{$lang.trusthistorynotes}
				</div>
				...
				<div class="wbox_content">
					<table width="100%" cellspacing="0" cellpadding="0" border="0" class="checker">
						...
						{foreach from=$status.notes_list item=entry name=notes}
							<tr class="{if $smarty.foreach.notes.index%2==0}even{/if} {if $smarty.foreach.notes.last}lastone{/if}">
