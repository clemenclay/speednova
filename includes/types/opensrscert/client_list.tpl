...
<table cellspacing="0" cellpadding="0" border="0" width="100%" class="styled table table-bordered table-striped">
		...
		{foreach from=$services item=service name=foo}
			<tr {if $smarty.foreach.foo.index%2 == 0}class="even"{/if}>
				...
				<td align="center" class="fs11">
					...
					?cmd=clientarea&amp;action={$action}&amp;service={$service.id}&amp;edo=Synchronize*}" class="fs11 lmore">{$lang.synchronize}</a>
