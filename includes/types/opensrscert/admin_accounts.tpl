...
{else}
	...
	{if $details.status=='Pending' || ($details.status=='Active' && ($details.cert_status=='awaiting-approval' || $details.cert_status=='completed' || $details.cert_status=='active'))}
	...
	<ul class="accor">
		<li><a href="#">OpenSRS Certificate Info</a>
			<div class="sor">
				<table cellspacing="2" cellpadding="3" border="0" width="100%" >
					<tbody>
						<tr >
							<td width="150">OpenSRS Product ID</td>
							<td >
						...
						<tr>
							<td width="150">OpenSRS Order ID</td>
							<td >
						...
						<tr>
							...
							<td >
								...
								{/if}
								{if $details.cert_status=='completed' || $details.cert_status=='active'}
									<input type="submit" value="Renew" onclick="$('body').addLoader(); $(this).after('<input type=\'hidden\' name=\'edo\' value=\'renew\' />')"/>
								{/if}
								{if $details.cert_status=='completed' || $details.cert_status=='active'}
									<input type="submit" value="Resend Cert" onclick="$('body').addLoader(); $(this).after('<input type=\'hidden\' name=\'edo\' value=\'resendcert\' />')"/>
								{/if}
							</td>
						</tr>
						<tr>
							...
							<td >
								...
								{if $details.cert_status=='awaiting-approval'}
									<input type="submit" name="edo" value="Resend" onclick="$('body').addLoader();"/>
								{/if}
