            ...
            <table cellspacing="0" cellpadding="0" border="0" width="100%" class="checker table table-striped">
                ...
                {foreach from=$DNSmanagement.hosts key=number item=host name=frea}
                    <tr {if $smarty.foreach.frea.iteration%2==0}class="even"{/if}>
                        ...
                        <td>
                            <input type="text" style="width: 20px" size="3" {if $host.recordtype != 'MX' && $host.recordtype != 'SRV'}disabled="disabled"{/if} value="{$host.priority}" name="updateDNSmanagement[Priority{$number}]"/>
                        </td>
                        ...
                        <td><a class="btn deleteico" href="#" onclick="record_toggle(this); return false;"><i class="icon icon-trash delbtn"></i></a></td>
                    </tr>
                {/foreach}
                <tr><td colspan="5"><strong>{$lang.newrecord}:</strong></td></tr>
                <tr>
                    <td><input type="text" class="styled" style="display: block;width: 95%" size="15" value="" name="updateDNSmanagement[newHostName]"/></td>
                    <td>
                        <input type="text" style="width: 20px" {if $DNSmanagement.records[0] != 'MX' && $DNSmanagement.records[0] != 'SRV'}disabled="disabled"{/if} size="3" name="updateDNSmanagement[newPriority]"/>
                    </td>
                    ...
                    <td colspan="2"><input type="text" class="styled" style="display: block;width: 95%" size="15" name="updateDNSmanagement[newAddress]"/></td>
                </tr>
