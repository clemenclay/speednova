    ...
    <tbody id="updater">
        ...
        <tr >
             <td >{if $vm.built=='true'}{if $vm.power=='true'}
                <a href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=poweroff&vpsid={$vm.id}&security_token={$security_token}" class="iphone_switch_container iphone_switch_container_on" onclick="return powerchange(this,'Are you sure you want to Power OFF this VM?');"><img src="includes/types/onappsmarttype/images/iphone_switch_container_off.png" alt="" /></a>

		{else}
                <a href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=poweron&vpsid={$vm.id}&security_token={$security_token}" class="iphone_switch_container iphone_switch_container_off" onclick="return powerchange(this,'Are you sure you want to Power ON this VM?');"><img src="includes/types/onappsmarttype/images/iphone_switch_container_off.png" alt="" /></a>

		{/if}{else}
                <a  class="iphone_switch_container iphone_switch_container_pending"><img src="includes/types/onappsmarttype/images/iphone_switch_container_off.png" alt="" /></a>

                 {/if}

            </td>
            <td><a href="?cmd=clientarea&action=services&service={$service.id}&vpsid={$vm.id}&vpsdo=vmdetails" ><b class="left">{$vm.hostname}</b> {if $vm.tags}{foreach from=$vm.tags item=tag}<span class="label label-success right fs11 " style="margin-right:2px">{$tag}</span> {/foreach}{/if}<span class="clear"></span></a></td>
