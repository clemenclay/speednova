    ...
    <tbody id="updater">
        ...
        {foreach from=$services item=service name=foo}
            <tr {if $smarty.foreach.foo.index%2 == 0}class="even"{/if} >
                {if $service.options.option10 == 'Single Machine, autocreation'}
                ...
                {else}
                    <td colspan="3">
                        <a href="?cmd=clientarea&amp;action={$action}&amp;service={$service.id}" style="text-decoration:none">
                            <strong  class="left">{$service.name}</strong> 
                            {if $service.tags}
                                {foreach from=$service.tags item=tag}
                                    <span class="label label-success right fs11 ">{$tag}</span> 
                                {/foreach}
                            {/if}
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            $('.load_vm_status').each(function() {
                var el = $(this).html("<img src='includes/types/openstacktype/images/ajax-loader.gif' />");
