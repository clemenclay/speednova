...
<table class="data-table backups-list"  width="100%" cellspacing=0>
    ...
    <tr>
        <td align="center">{if $interface.primary=='true'}<img src="includes/types/openstacktype/images/icons/tick.png" alt="Primary"/>{else}{/if}</td>
        <td >{$interface.label}</td>
        <td>{if $interface.rate_limit=='0'}Unlimited{else}{$interface.rate_limit} Mbps{/if}</td>
        <td>
