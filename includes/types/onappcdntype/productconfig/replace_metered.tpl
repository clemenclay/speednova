...
<div id="meteredmgr" >
    <div style="padding: 5px;">
        ...
        <br/> Note: For CDN you can override Traffic pricing below by Edge Group specific pricing - if edgegroup form element is added & pricing set. <strong>Edge group prices are billed using unit scheme</strong>
                                ...
                                <tbody>
                                    ...
                                    <tr  id="var_{$k}"  {if !$varx.enable }style="display:none"{/if}>
                                        ...
                                        <td colspan="2">
                                            ...
                                            <div style="{if !$varx.enable || $varx.scheme!='unit'}display:none;{/if} padding: 10px 0px;min-width: 285px;" id="var_{$k}_unit" class="left">
                                                <input type="hidden"  value="0" name="metered[{$k}][prices][0][qty_max]"  />
                                                <input value="0" name="metered[{$k}][prices][0][qty]"  type="hidden"/>
                                                Price per hour: 
                                                <input type="text" class="inp" size="4" 
                                                    value="{if !$varx.prices}{0.000|price:$currency:false:false}{else}{$varx.prices[0].price|price:$currency:false}{/if}"  
                                                    name="metered[{$k}][prices][0][price]" /> per 1  {$varx.unit_name}
                                            </div>
                                            <div {if !$varx.enable || $varx.scheme=='unit'}style="display:none"{/if} id="var_{$k}_scheme" class="left" >
                                                ...
                                                <div style="padding:15px 0px 10px;">
                                                    <a onclick="return addBracket('{$k}', ' /{$varx.unit_name}');"   class="prices menuitm " href="#">
                                                        <span  class="addsth">Add price bracket</span>
                                                    </a>
                                                </div>
                                            </div>
                                                        <div class="scheme_container" style="margin: 0 0 0 20px">
                                                Pricing scheme:
                                                <select class="inp m_scheme" name="metered[{$k}][scheme]" data-var="{$k}">
                                                    <option {if $varx.scheme=='unit'}selected="selected"{/if} value="unit">Unit</option>
                                                ...
                                                <a class="vtip_description" title=" <b>Overage tiers</b><br/> Use this scheme as tiered, where each expected measurement is low (i.e. hourly bandwidth).<br/> i.e.: <i>0-100: $0, 101-150: $1 </i>:<br/> up to 100 - free, everything above 100 - $1/unit. So total of: 102 measured will cost $2<br/><br/>"></a>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
            ...
            <table width="100%" cellspacing="0" cellpadding="6" border="0">
                <tr>
                    <td width="160" >Billing type <a title="<b>Post Pay</b> - Client pays for used resources by the end of billing period<br><br>
                                                     ...
                                                     <br>Once account credit reach 0 account will be suspended
                ...
                <tr>
                    ...
                    <td >
                        <select name="config[MeteredCycle]" class="inp" id="metered_invoice_generation">
...
{literal}
    <script type="text/javascript">
        $('.m_scheme').change(function(){
            var self = $(this),
                id = self.data('var'),
                scheme =  $('#var_' + id + '_scheme'),
                unit = $('#var_' + id + '_unit');
                
            if ($(self).val() == 'unit') {
                scheme.hide().find('select, input, textarea').prop('disabled', true);
                unit.show().find('select, input, textarea').prop('disabled', false);
            } else {
                scheme.show().find('select, input, textarea').prop('disabled', false);
                unit.hide().find('select, input, textarea').prop('disabled', true );
            }
        }).change();
