...
<div class="header-bar">
    <h3 class="{$cdndo} hasicon">Billing</h3>
    <div class="clear"></div>
</div>
<div class="content-bar ">
    ...
    <div class="mseparator"></div>
            {if $service.metered && $service.showbilling && ($service.status=='Active' || $service.status=='Suspended')}
              ...
              {if $service.metered_type!='PrePay'} <h3 class="summarize">{$lang.nextinvoicedetails} <span>(updated hourly)</span></h3>{/if}
          ...
          {/if}
            ...
            <link rel="stylesheet" href="includes/types/onappcdntype/clientarea/demo_table.css" type="text/css" />
        ...
        {literal}    <script type="text/javascript">
                ...
                function metteredBillinghistory() {
                            ...
                            var url={/literal}'?cmd=clientarea&action=services&service={$service.id}&cdndo=billing&do=metered_history';{literal};
    ...
    <div class="mseparator"></div>
        ...
        <div id="pricingtable"> {include file="`$cdndir`metered_table.tpl"}</div>
