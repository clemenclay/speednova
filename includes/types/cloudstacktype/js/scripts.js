...
var init_sliders = function() {
    $(".slider").each(function(index, object) {
        ...
        if(fmin = slider.attr("fmin"))
            fmin = parseInt(fmin.toString())
        ...
        if (target.length)
        {
            ...
            var update_target = function() {
                var slidervalue = parseInt(slider.slider("value").toString()) || 0;
                var total = slider.attr("total");
                
                if(fmin > slidervalue)
                    slidervalue = fmin;
                
                target.attr("value", slidervalue);
                ...
                if (divide.length && total) {
                    var tot = parseInt(total / (parseInt(target.attr("value").toString()) || 0)),
                        uplim = parseInt(divide.attr('up-limit')) || tot;
                    if(tot > uplim)
                        tot = uplim;
        ...
        // enfore steps #after initial value
        if (step = slider.attr("step"))
            slider.slider("option", "step", parseInt(step.toString()));
    });
    oschange($('#virtual_machine_template_id'));
}

function filtertemplates(place) {
    ...
    for (var i = 0; i < ostemplates.length; i++) {
        if (ostemplates[i].family == family && ostemplates[i].distro == distro) {
            $('#virtual_machine_template_id').append('<option '+(ostemplates[i].disabled.length ? 'disabled="'+ostemplates[i].disabled+'"' : '')+' value="' + ostemplates[i].id + '">' + ostemplates[i].name + ' ' + ostemplates[i].disabled +'</option>');
        }
    }
    $('#virtual_machine_template_id').selectedIndex = 0;
    $('#virtual_machine_template_id').change();
    swapcheck($('#virtual_machine_template_id').val());
    $(document).trigger('filtertemplates');
}
...
function oschange(that){
    that = $(that);
    var is_root = that.data('root'),
        value = that.val(),
        opt = that.children('[value='+value+']'),
        dataSize = parseInt(opt.attr('data-size')),
        dataslider = $('#disk-row .slider'),
        instance = $('#instance-row .slider'),
        indi = $('#storage_limit_indicator'),
        inval = parseInt(indi.text().replace(/[^\d]+/g, '')),
        disabled = opt.is(':disabled');
    
    
    $('#disk-row .slabel').hide()
        .filter('.for-' + (isNaN(dataSize) ? 'iso' : 'template:last')).show();

    if(!opt.length)
        return;
    if(disabled){
        indi.css('color','red');
        that.val(opt.siblings('not(:disabled):first').attr('value')).change();
        return;
    }
    
    var min = parseInt(dataslider.attr('min').toString()) || 0,
        max = parseInt(dataslider.attr('max').toString()) || 0,
        limit = parseInt(dataslider.attr('total').toString()) || 0;
    
    if(!dataslider.length)
        return;
    
    if(instance.length){
        var instanceinitial = instance.data("ini-total") || instance.attr("total");
        instance.data("ini-total", instanceinitial)
    }
    
    if(isNaN(dataSize)){
        dataslider.slider('option','min', min);
        dataslider.slider('option','max', max);
        dataslider.slider("value", dataslider.slider("value"))
        ...
        if(is_root){
            dataslider.slider('option','min', dataSize + min);
            dataslider.slider('option','max', max);
            dataslider.slider("value", dataslider.slider("value"))
            
            if(instance.length){
                instance.attr("total", instance.data("ini-total"));
            }
        }else{
            dataslider.slider('option','min', min);
            dataslider.slider('option','max', max - dataSize);
            dataslider.slider("value", dataslider.slider("value"))
            
            if(instance.length){
                instance.attr("total", instance.data("ini-total") - dataSize);
            }
        }
        if(dataSize > inval)
            indi.css('color','red');
    ...
    if(instance.length)
        instance.slider("value", instance.slider("value"));
}
...
function selecttplopt(o){
    if(!o)
        $('#os_templates').show();
    else $('#os_templates').hide();
}