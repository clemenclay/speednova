{if !$vm}
    <a class="iphone_switch {if $service.options.option10 == 'Single Machine, autocreation' && $service.status=='Active'}vm-status-pending{/if}" data-sid="{$service.id}" >
...
{elseif $vm.built=='true'}
    <a class="iphone_switch vm-status-pending" data-sid="{$service.id}" data-vid="{$vm.id}" title="Your server is changing power state, it might take few minutes." data-toggle="tooltip">
        {$lang.Pending}
    </a>
{else}
    <a class="iphone_switch vm-status-pending" data-sid="{$service.id}" data-vid="{$vm.id}" title="Your server is now being provisioned, it might take up to 15 minutes." data-toggle="tooltip">
        Creating
    </a>
{/if}