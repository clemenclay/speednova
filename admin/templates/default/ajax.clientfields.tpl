                    ...
                    <tr>
                        ...
                        <td class="conv_content form"  valign="top">
                            ...
                            <div class="tabb" style="display:none;overflow:visible">
                                ...
                                <label for="check-searchable"  class="havedescr">Searchable
                                    <small>Check this field value when serching for clients</small></label>
                                <input id="check-searchable" type="checkbox" name="options[]" value="1024" 
                                ...
                                <input id="check-adminonly" type="checkbox" name="options[]" value="128" {if $field.options & 128}checked="checked"{/if} onclick="{literal}if ($(this).is(':checked')) {
                                            $('#check-required, #check-editable, #check-invoice').removeAttr('checked');
                                        }{/literal}"/>
                                <div class="clear"></div>

                                <table border="0" cellspacing="0" cellpadding="0" width="100%">
                                    {if $field.code!='country'}<tr>
                                        <td width="160" valign="top"><label for="country"  class="havedescr">Limit for countries
                                                <small>Enter countries this field will appear for</small></label></td>
                                        <td align="left" valign="top" style="padding-left:10px">
                                            <select name="country[]" id="countryselector" multiple="multiple">
                                                {foreach from=$countries item=country key=c}
                                                    <option value="{$c}" {if $field.countries[$c]}selected="selected"{/if}>{$country}</option>
                                                {/foreach}
                                            </select>
                                            <div class="clear"></div>
                                        ...
                                        <td align="left" valign="top">
                                            ...
                                            <span class="right">
                                                <a href="#" class="editbtn" onclick="$(this).hide();
                                                        $('#premade_pattern').show();
                                                        return false;">Premade patterns</a>
    ...
    {if !$ajax}
        ...
        <div class="blu">
            <div class="left menubar">
                <a onclick="addCustomFieldForm();
                        return false;" class="new_control" href="#"><span class="addsth"><strong>{$lang.addnewfield}</strong></span></a>
                {if $premade} <a onclick="$('#premadefields').toggle();
                        return false;" class="new_control" href="#"><span class="dwd"><strong>Premade field profiles</strong></span></a>{/if}
                                    ...
                                    <table  cellpadding="0" cellspacing="0"  width="100%" >
                                        <tbody> 
                                            <tr class="havecontrols">
                                                {if $field.options & 1}
                                                    <td width="120" style="{if ($field.options & 16) || ($field.options & 256)}background:#F0F0F3;color:#767679;{/if}padding-left: 10px" >
                                                        <div class="btn-group">
                                                            <a class="sorter-ha btn btn-sm btn-default">
                                                                <i class="fa fa-sort"></i>
                                                            </a>
                                                            {if !($field.options & 16)}
                                                                <a class="menuitm btn btn-sm btn-default" href="?cmd=clients&action=field&field_id={$field.id}" 
                                                                   onclick="return editFieldForm('{$field.id}');" title="Edit"  >
                                                                    <i class="fa fa-pencil"></i>
                                                                </a>
                                                            {/if}
                                                            {if $field.options & 32}
                                                                <a class="menuitm btn btn-sm btn-default" title="Enable"  href="?cmd=clients&action=fields&make=delete&field_id={$field.id}" 
                                                                   onclick="return enable_field('{$field.id}');"><i class="fa fa-toggle-off"></i>
                                                                </a>
                                                            {elseif !($field.options & 64)}
                                                                <a class="menuitm menu-auto" title="Disable" href="?cmd=clients&action=fields&make=delete&field_id={$field.id}&system=1" 
                                                ...
                                                {else}
                                                    <td width="120" style="padding-left: 10px" >
                                                        <div class="btn-group">
                                                            ...
                                                            {if !($field.options & 64)}
                                                                <a class="btn btn-default btn-sm" title="Delete"  href="?cmd=clients&action=fields&make=delete&field_id={$field.id}" 
                                                                   onclick="return delete_field('{$field.id}', 0);"><i class="fa fa-trash-o"></i>
                                                                </a>
                                                            {/if}
                                                        </div>
                                                    </td>
                                                ...
                                                <td width="150" style="background:#F0F0F3;color:#767679;font-size:11px" >
                                                    <div style="padding:5px">
                                                        ...
                                                        {if $field.country}For countries: <b>{$field.country}</b>{/if}
                ...
                {literal}
                    ...
                    <script type="text/javascript">
                        ...
                        function editFieldForm(id) {
                            $.facebox({ajax: "?cmd=clients&action=field&field_id=" + id, width: 900, nofooter: true, opacity: 0.5, addclass: 'modernfacebox'});
                            return false;
                        }
                        function addCustomFieldForm() {
                            $.facebox({ajax: "?cmd=clients&action=field&field_id=new", width: 900, nofooter: true, opacity: 0.5, addclass: 'modernfacebox'});
                            return false;
                        }
