            ...
            <table cellspacing="0" cellpadding="3" border="0" width="100%" class="table glike hover">
                ...
                {foreach from=$items item=item}
                    <tr>
                        ...
                        <td>
                            {if $item.client_id}
                                <a href="?cmd=clients&action=show&id={$item.client_id}">{$item.clientname}</a>
                            {else}
                                {$item.clientname}
                            {/if}
                        </td>
                    ...
                    <tbody>
                        ...
                        {foreach from=$items item=entry}
                            ...
                            <tr>
                                ...
                                <td>
                                    <div><strong>{$entry.name}</strong> <small>({$entry.rate|price:$currency:true:2:false}{if $entry.type =='hourly'}/h{/if})</small></div>
                                ...
                                <td>
                                    ...
                                    {elseif $entry.status == 1}
                                        <span class="Active">Invoiced 
                                            <small>
                                                <a href="?cmd=invoices&action=edit&id={$entry.invoice_id}&list=all" style="color: #4D89AB" >#{$entry.invoice_id}</a>
                                            </small>
                                        </span>
            ...
            <div class="row ">
                ...
                <div class="col-lg-4 col-sm-6">
                    <div class="form-group">
                        ...
                        <select class="form-control chosenedge" name="rate_id" s>
                            {foreach from=$rates item=rate}
                                <option {if $submit.rate == $rate.id}selected="selected"{/if} value="{$rate.id}">
                                    {$rate.name} ({$rate.price|price:$currency:true:2:false}{if $rate.type == 'hourly'}/h{/if}) 
                                </option>
                            {/foreach}
                        </select>
                    </div>
                ...
                <div class="col-lg-3 col-sm-6">
                    <div class="form-group">
                        <label>Note</label>
                        <textarea name="note" class="form-control" style="height: 2.6em;"></textarea>
                    </div>
                </div>
            ...
            {if $pendingitems && !$forbidAccess.addInvoices && !$forbidAccess.editInvoices}
                ...
                <div class="row">
                    ...
                    <div class="col-lg-2 col-sm-12 form-group" >
                        ...
                        <button class="btn btn-default" onclick="ticket.startBilling($('#billtype').val(), $('#billingservices').val());
