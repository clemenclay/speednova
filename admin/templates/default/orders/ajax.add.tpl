            ...
            <tr class="havecontrols service_{$sid} {counter name=alter}{if $alter%2==0}even{/if}" >
                ...
                <td>
                    ...
                    {else}
                        ...
                        {/if}
                    {/if}
                    {if !$service.org_price && ($service.addons ||  $service.customizations)} 
            ...
            {if $service.addons}
                {foreach from=$service.addons item=addon}
                    <tr {if !$service.org_price && $fold.service.$sid}style="display:none"{/if} class="foldable havecontrols service_{$sid} {if $alter%2==0}even{/if}">
                        ...
                        <td class="editbtn_flash">
                            <span {if $addon.org_price}class="pricemod" title="This price was modified"{/if}>{$addon.price|price:$draft.currency}</span>
                            <input style="display:none; width: 98px"  class="custom_product_price" type="text" 
                    ...
                    {if $c.items}
                        <tr style="vertical-align: top; {if !$service.org_price && $fold.service.$sid}display:none{/if}" class="foldable havecontrols service_{$sid} {if $alter%2==0}even{/if}">
                            ...
                            <td class="service_{$sid}_custom_{$kk}">
                                {include file=$c.configtemplates.accounts currency=$draft.currency forcerecalc=true cf_opt_name="service[`$sid`][custom]"} [{$sid} {$c.qty}]
                            ...
                            <td class="editbtn_flash">
                                {foreach from=$draft.customizations[$sid][$c.id] item=ci key=cfk}
                                    ...
                                    <span {if $c.items[$cfk].org_price}class="pricemod" title="This price was modified"{/if}>{$c.price|price:$draft.currency}</span>
                                    <input style="display:none; width: 98px"  class="custom_product_price" type="text" 
                                        name="service[{$sid}][custom_cdata][{$c.id}][{$cfk}][billing][price]" value="{if $c.items[$cfk].org_price}{$c.price}{/if}" 
                                        placeholder="{if $c.items[$cfk].org_price}{$c.items[$cfk].org_price|price:$draft.currency}{else}{$c.items[$cfk].price|price:$draft.currency}{/if}" />
                                    <a href="#" class="editbtn" onclick="return order.edit_back(this);" >{$lang.Edit}</a>
                                {/foreach}
                            </td>
                                        ...
                                        {if $clients} 
                                            {foreach from=$clients item=cl}
                                                {if $draft.client_id == $cl.id}
                                                    <a href="?cmd=clients&action=show&id={$draft.client_id}">{$cl.firstname} {$cl.lastname}</a>
                                                {/if}
                                            {/foreach}
                                        {else}
                                ...
                                {if $clients} 
                                    <span {if $draft.client_id}style="display:none"{/if} class="editbtn_flash">
                                        <select class="inp" name="client_id" load="clients" default="{$draft.client_id}" style="width: 180px">
                                            ...
                                            {*foreach from=$clients item=cl}
                                                <option {if $draft.client_id == $cl.id}selected="selected"{/if} value="{$cl.id}">#{$cl.id} {if $cl.companyname!=''}{$lang.Company}: {$cl.companyname}{else}{$cl.firstname} {$cl.lastname}{/if}</option>
                                            {/foreach*}
                                        </select>
                                        <a href="#" class="editbtn saved" style="visibility: visible" onclick="order.save_details(); return false;">{$lang.Save}</a>
                                    </span>
