                ...
                <form action="?cmd={$cmd}&action={$action}&make=add" method="post" onsubmit="order.edit(this); return false;">
                    ...
                    <div class="p6 secondtd">
                        ...
                        <a class="btn btn-warning btn-sm" href="?cmd={$cmd}&action={$action}&id={$draft.id}&make=delete" 
                           onclick="return confirm('{$lang.deleteorderconfirm}');" >{$lang.deldraft}</a>    
                        &nbsp;
                        <small>{$lang.whenfinished}</small>
                        <a class="btn btn-primary btn-sm" href="?cmd={$cmd}&action={$action}&id={$draft.id}&make=create" 
                           onclick="return order.confirm_unsaved('There are some unsaved changes, do you want to continue?')" >{$lang.generatefromdraft}</a>
                           
                        <a class="btn btn-info btn-sm" href="?cmd={$cmd}&action={$action}&id={$draft.id}&make=estimate" 
                           onclick="return order.confirm_unsaved('There are some unsaved changes, do you want to continue?')" >Generate Estimate from this draft</a>
