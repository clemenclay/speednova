<div class="blu"> 
    ...
    {/if}
    {if $action=='add'}
        &raquo; <strong>{if $pre}Add Pre Filter{else}Add Post Filter{/if}</strong>
    {/if}
    {if $action=='edit'}
        &raquo; <strong>{if $pre}Pre{else}Post{/if} {$lang.filter}</strong>
    {/if}
</div>
                    ...
                    {if $filter.items}
                        {foreach from=$filter.items item=item name=loop}
                            <tr {if $smarty.foreach.loop.last}class="last"{/if}>
                                ...
                                <td style="vertical-align: top;">
                                    <textarea class="inp" style="width:500px; height:17px; box-sizing: content-box;" name="item_content[]">{$item.content}</textarea>
                                </td>
        ...
        <div class="list-1">
            <ul>
                <li {if !$pickedtab}class="active"{/if}>
                    <a href="#"><span class="ico money">Pre {$lang.filters}</span></a>
                </li>
                <li class="last">
                    <a href="#"><span class="ico plug">Post {$lang.filters}</span></a>
                </li>
            </ul>
        </div>
        <div class="list-2">
