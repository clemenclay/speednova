...
{if $action=='gettop'}
    <div id="replybrocontainer">
        {if $replies}
            <div class="pull-right" class="fs11"><a href="?cmd=predefinied&action=myreplies" target="_blank" class="editbtn" >{$lang.manageyour}</a></div>
            {foreach from=$replies item=reply key=k}
                <div  class="replyget">
                    <a href="?cmd={$cmd}&action=getmacro&id={$reply.id}" title="{$reply.name|escape}"><strong>{$reply.name|truncate:100|escape}</strong></a> 
                    <i style="color:#999">{$reply.reply|truncate:100|escape}</i>
                </div>
            {/foreach}
        {else}
...
*}{elseif $action=='new' || $action=='view'}
    ...
    <div class="nicerblu" style="padding:10px;">
        <table border="0" cellpadding="3" cellspacing="0" width="100%">
            ...
            <tr>
                <td><b>{$lang.Name}:</b></td>
                <td><input name="name" value="{$reply.name|escape}" size="70" class="inp"/></td>
            ...
            <tr>
                ...
                <td>
                    <input type="checkbox" name="notify" value="1" {if $reply.notify}checked="checked"{/if}> Send email notifications about Ticket reply
                </td>
            </tr>
