    ...
    <form method="post" {if $action=='add'}action="?cmd=emailtemplates&action=add"{else}action="?cmd=emailtemplates&action=edit&id={$email.id}"{/if}  {if $inline} onsubmit="return submitform(this)"{/if}>
        ...
        {if !$inline}
            <div class="blu"> 
                ...
                {/if}
                <input type="submit" class="btn btn-primary btn-sm" name="submit" style="font-weight:bold" value="{if $action=='add'}{$lang.addtemplate}{else}{$lang.savechanges}{/if}" />
                <input type="button" onclick="window.location = '?cmd=emailtemplates'" value="{$lang.Cancel}" class="btn btn-default btn-sm" />
                {if $email.system=='0'}
                    <input class="btn btn-danger btn-sm"  type="button" onclick="{literal}if (confirm('{/literal}{$lang.deletetemplateconfirm}{literal}')) {
                                window.location = '?cmd=emailtemplates{/literal}&security_token={$security_token}{literal}&make=delete&id={/literal}{$email.id}{literal}'
                            } else {
                                return false;
                            }{/literal}"  value="{$lang.Delete}"/>
                {/if}
            </div>
        {/if}
        ...
        <div class="blu"> 
            ...
            <input type="submit" name="submit" style="font-weight:bold" value="{$lang.savechanges}"class="btn btn-primary btn-sm" />
            {if !$inline}
                <input type="button" onclick="window.location = '?cmd=emailtemplates'" value="{$lang.Cancel}" class="btn btn-default btn-sm"/>
            {/if}
            {if !$inline}
                {if $email.system=='0'}
                    <input type="button" class="btn btn-danger btn-sm" onclick="{literal}if (confirm('{/literal}{$lang.deletetemplateconfirm}{literal}')) {
                            window.location = '?cmd=emailtemplates{/literal}&security_token={$security_token}{literal}&make=delete&id={/literal}{$email.id}{literal}'
                        } else {
                            return false;
                        }{/literal}"  value="{$lang.Delete}"/>
