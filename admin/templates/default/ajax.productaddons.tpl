							...
							{/if}
                                ...
                                <form id="serializeit" method="post">
                                    ...
                                    <ul id="grab-sorter" >
                                        ...
                                        <li {if $a.visible=='0'}class="ishidden"{/if}><div><table width="100%" cellspacing="0" cellpadding="3" border="0" class="table glike">
                                                    <tr  class="havecontrols">
                                                        <td  width="14" valign="middle"><div class="controls"><a class="sorter-handle">sort</a></div>&nbsp;</td>
                                                        <td width="31%"><input type="hidden" name="sorts[]" value="{$a.id}" />{if $a.visible=='0'}<em class="ishidden">{/if}<a href="?cmd=productaddons&action=addon&id={$a.id}">{$a.name}</a>{if $a.visible=='0'}</em>{/if} {if $a.visible=='0'}<em class="hidden fs11">{$lang.hidden}</em>{/if}</td>
