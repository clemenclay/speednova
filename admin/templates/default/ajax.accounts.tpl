...
<div class="fs11" style="text-align: right;padding-top: 5px">
    ...
    <div class="btn-group pull-left">
        <a href="#" class="btn btn-default btn-sm" onclick=" ajax_update('?cmd=accounts&action=getqueue&id={$account_id}&account_id={$account_id}&product_id={if $pid}{$pid}{else}{$details.product_id}{/if}', false, '#autoqueue', true); return false">Refresh</a>
        <button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            Schedule task <span class="caret"></span>
        </button>
        <ul class="dropdown-menu">
            <li><a href="?cmd=tasklist&action=assigntask&id={$account_id}&place=accounts" onclick="return assignNewTask({$account_id},'accounts')">For this account</a></li>
            <li><a  href="?cmd=services&action=product&id={$pid}&picked_tab=2" target="_blank">For this product</a></li>
        </ul>
    </div>
...
{/literal}
	 ...
	 <table border="0" cellpadding="2" cellspacing="0" width="100%" class="table glike" id="addontbl" {if !$account_addons}style="display:none"{/if}>
...
<center>
	...
	<input type="submit" name="changeowner" value="{$lang.changeowner}"  style="font-weight:bold" class="btn btn-primary btn-sm"/>

	<input type="button" value="{$lang.Cancel}" onclick="$('#ChangeOwner').hide();return false;" class="btn btn-default btn-sm"/>
        ...
        {if $custom_filters}
            <table width="100%" cellspacing="2" cellpadding="3" border="0">
                <tbody>
                    ...
                    &nbsp;&nbsp;&nbsp;
                    <input type="submit" value="{$lang.Cancel}" onclick="$('#hider2').show();$('#hider').hide();return false;" class="btn btn-default btn-sm"/>
                </center></td>
                </tr>
                </tbody>
            </table>
    ...
    {elseif $action=='clientaccounts'}
        <div class="blu clearfix">
            ...
            <div class="pull-right">
                ...
                <a href="?cmd=orders&action=add&related_client_id={$client_id}" class="btn btn-primary pull-right btn-xs" target="_blank"><i class="fa fa-plus"></i> {$lang.newaccount}</a>
            </div>

            <form action="" method="post" class="pull-left">
                Category:
                <select id="client_service_cat" name="category_id" style="width:250px;" onchange="$('.slide:visible').addLoader();ajax_update('',$(this).parents('form').eq(0).serializeArray(),$(this).parents('.slide'));return false;">
                    {foreach from=$categories item=c}
                        <option value="{$c.id}" {if $category_id==$c.id}selected="selected"{/if}>{$c.name} ({$c.total})</option>
                    {/foreach}
                </select>
                {$lang.status}
                <select id="client_service_status" name="status" onchange="$('.slide:visible').addLoader();ajax_update('',$(this).parents('form').eq(0).serializeArray(),$(this).parents('.slide'));return false;">
                    ...
                    <option value="Terminated" {if $status=="Terminated"}selected="selected"{/if}>{$lang.Terminated}</option>
        ...
        {if $accounts}
            <table cellspacing="0" cellpadding="3" border="0" width="100%" class="table glike hover">
                <thead>
                    <tr>
                        <th class="text-nowrap"><a href="?cmd=accounts&action=clientaccounts&id={$client_id}" class="sortorder" data-orderby="id">{$lang.accounthash}</a></th>
                        <th><a href="?cmd=accounts&action=clientaccounts&id={$client_id}" class="sortorder" data-orderby="domain">{$lang.Domain}</a></th>
                        <th><a href="?cmd=accounts&action=clientaccounts&id={$client_id}" class="sortorder" data-orderby="name">{$lang.Service}</a></th>
                        <th><a href="?cmd=accounts&action=clientaccounts&id={$client_id}" class="sortorder" data-orderby="total">{$lang.Price}</a></th>
                        <th><a href="?cmd=accounts&action=clientaccounts&id={$client_id}" class="sortorder" data-orderby="billingcycle">{$lang.billingcycle}</a></th>
                        <th><a href="?cmd=accounts&action=clientaccounts&id={$client_id}" class="sortorder" data-orderby="status">{$lang.Status}</a></th>
                        <th><a href="?cmd=accounts&action=clientaccounts&id={$client_id}" class="sortorder" data-orderby="next_due">{$lang.nextdue}</a></th>
                        <th width="20"/>
                    </tr>
                </thead>
            ...
            {if $totalpages}
                <center class="blu paginercontainer" >
                    <strong>{$lang.Page} </strong>
                    {section name=foo loop=$totalpages}
                        <a href='?cmd=accounts&action=clientaccounts&id={$client_id}&page={$smarty.section.foo.iteration-1}&category_id={$category_id}&status={$status}' class="npaginer
                    {if $smarty.section.foo.iteration-1==$currentpage}
                    currentpage
                    {/if}"
                        >{$smarty.section.foo.iteration}</a>
