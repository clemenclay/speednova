        ...
        <tr  id="pricing_{$p.period}" class="havecontrols" >
            ...
            <td><div class="e1">
                ...
                <div class="e2">
                    <table border="0" cellpadding="3" cellspacing="0">
                        <tr>
                            <td >{$lang.Register}: <input type="checkbox"  value="1"   onclick="check_i(this)" {if $p.register!=-1}checked="checked"{/if} /><input type="text" value="{$p.register|price:$currency:false}" size="6" name="periods[{$p.period}][register]" class="config_val inp" /></td>
                            <td >{$lang.Transfer}: <input type="checkbox"  value="1"   onclick="check_i(this)" {if $p.transfer!=-1}checked="checked"{/if} /><input type="text" value="{$p.transfer|price:$currency:false}" size="6" name="periods[{$p.period}][transfer]" class="config_val inp" /></td>
                            <td >{$lang.Renew}: <input type="checkbox"  value="1"  onclick="check_i(this)" {if $p.renew!=-1}checked="checked"{/if} /><input type="text" value="{$p.renew|price:$currency:false}" size="6" name="periods[{$p.period}][renew]" class="config_val inp" /></td>
                        </tr>
                    </table>
                </div></td>
        </tr>
    ...
    <div >
        <table border="0" cellpadding="3" cellspacing="0" width="100%">
            <tr>
                ...
                <td ><input id="pediodid" size="6" value="1" /> {$lang.Years}</td>

                <td ><strong>{$lang.Register}</strong><input type="checkbox"  value="1"  id="register_on" onclick="check_i(this)" checked="checked"/> <input id="register_p" value="{0|price:$currency:false:false}" size="6" class="config_val price inp"/></td>
                <td ><strong>{$lang.Transfer}</strong><input type="checkbox"  value="1"  id="transfer_on" onclick="check_i(this)" checked="checked"/> <input id="transfer_p" value="{0|price:$currency:false:false}" size="6" class="config_val price inp"/></td>
                <td ><strong>{$lang.Renew}</strong><input type="checkbox"  value="1"  id="renew_on" onclick="check_i(this)" checked="checked"/> <input id="renew_p" value="{0|price:$currency:false:false}" size="6" class="config_val price inp"/></td>

                <td colspan="2" align="center"><input type="button" value="{$lang.Add}" class="btn btn-success btn-xs"  onclick="addopt_domain();return false;"/> <span class="orspace">{$lang.Or} </span> <a href="#" onclick="$('#addpricingd').show();$('#tbpricingd').hide();return false;" class="editbtn" id="hidepricingadd">{$lang.Cancel}</a></td>
        </table>
    </div>
</div>
{literal}
