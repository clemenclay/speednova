                ...
                <tr>
                    ...
                    {foreach from=$agreement.actions item=status name=actions}
                        ...
                        <a class="menuitm menu-auto" 
                            href="?cmd={$cmd}&action={$action}&changestatusto={$status}&uuid={$agreement.uuid}&security_token={$security_token}"
                            {if $status =='accepted'}{assign value="1" var=showaccept} onclick="$('#h{$agreement.uuid}').fadeToggle();$(this).toggleClass('activated'); return false;" 
                            {elseif $status =='delete'}onclick="return confirm('{$lang.areyousure}')"
                            {/if}
                          >{$lang.$prefact}</a>
                    {/foreach}
                    </td>
