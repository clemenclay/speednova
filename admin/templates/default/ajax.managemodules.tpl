                                                        ...
                                                        <div style="text-align:right;padding:5px 0px">
                                                            {if $b.type=='Payment' || $b.type=='Hosting' || $b.type=='Domain' || $b.type=='Fraud'}
                                                                ...
                                                                {if (!empty($b.config) && $b.type!='Hosting' && $b.type!='Domain' ) || $b.type=='Payment' } 
                                                                    <a class="menuitm menu-auto {if $expand==$b.id}activated{/if}"   href="#" onclick="$('#config-row-{$b.id}').toggle();
                                                            ...
                                                            {if  $b.currencies}
                                                                ...
                                                                <td ><a style="padding: 5px 12px 0 10px; background-position: center center;" class="vtip_description" title="When enabled, gateway will appear in all currencies - even unsupported by this gateway.
                                                                     ...
                                                                     <select name="option[background_conversion]">
                                                                        {assign var="cursplit" value=","|explode:$b.currencies}

                                                                        <option value="" {if !$b.config.background_conversion.value}selected="selected"{/if}>None. Disable conversion</option>
                                                                        {foreach from=$cursplit item=cur}
                                                                            <option value="{$cur|trim}" {if $cur|trim == $b.config.background_conversion.value}selected="selected"{/if}>{$cur|trim}</option>
                                                                        {/foreach}
                                                                    </select>
