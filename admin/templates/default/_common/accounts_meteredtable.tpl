<table class="whitetable" width="100%" cellspacing="0" cellpadding="3">
    ...
    <tbody>
        ...
        <tr class="odd">
            <td colspan="{$metered_usage_log.colspan}" align="center">No usage recorded yet</td>
        </tr>
        {else}
        ...
        {else}
        <tr class="{if $smarty.foreach.usageloop.iteration%2==0}even{else}odd{/if}">
            <td colspan="{$metered_usage_log.colspan}" style="background:#ecf5ff"><em>{$val.date} - {$val.type} Report sent</em></td>
        </tr>
        {/if}
        {/foreach}
        {/if}
