    ...
    <tr>
        <td width="50%" valign="top">
            ...
            <table cellspacing="2" cellpadding="3" border="0" width="100%" >
                <tbody>
                    ...
                    <tr>
                        ...
                        <td >
                            ...
                            <a href="?cmd=services&action=product&id={$details.product_id}" target="_blank" id="link_to_product" class="btn btn-sm"><i class="fa fa-external-link"></i></a>
                            <div style="display:none;padding:3px;" id="upgrade_opt" class="lighterblue">

                                <input type="submit" class="btn btn-default btn-sm" name="charge_upgrade" value="{$lang.orderupgrade}" {if  $details.status=='Pending' || $details.status=='Terminated'}style="display:none"{/if}/>
                                {if $allowpckgchange}<input type="submit" class="btn btn-default btn-sm" name="changepackage" onclick="return checkup()" value="{$lang.changepackage}" class="livemode" {if  $details.status=='Pending' || $details.status=='Terminated'}style="display:none"{/if}/>{/if}
                            </div>
                    ...
                    {if !$details_fields || (count($details_fields)==1 && $details_fields.option4)}
                        {if $details.domain!=""}
                            <tr>
                                ...
                                <td >
                                    ...
                                    <a style="color: rgb(204, 0, 0);" target="_blank" href="http://{$details.domain}">www</a> <a target="_blank" href="{$system_url}?cmd=checkdomain&action=whois&domain={$details.domain}" onclick="window.open(this.href, 'WHOIS', 'width=500, height=500, scrollbars=1');
                        ...
                        {if $details.ptype=='Dedicated' || $details.ptype=='Server' || $details.ptype=='Colocation'}
                            ...
                            <tr>
                                <td >{$lang.Password}</td>
                                <td ><input type="text" value="{$details.password}" name="password" size="20" id="password"/>  {if $allowpasschange}<input type="submit" name="changepassword" class="btn btn-default btn-sm" value="{$lang.changepassword}" />{/if}</td>
                            </tr>
                            <tr>
                                <td >{$lang.rootpass}</td>
                    ...
                    {elseif $details_fields}
                        ...
                        {elseif $field.name == 'password'}
                            ...
                            {else}
                                <tr>
                                    <td >{$lang.Password}</td>
                                    <td ><input type="text" value="{$details.password}" name="password" size="20" id="password"/>  {if $allowpasschange}<input type="submit" name="changepassword" value="{$lang.changepassword}" class="btn btn-default btn-sm" />{/if}</td>
                                </tr>
                            {/if}
                ...
                <tr {if !$allowterminate && !$allowsuspend && !$allowcreate && !$allowunsuspend && !$allowrenewal}class="manumode" {if $details.manual!='1'}style="display:none"{/if}{/if}>
                    <td >{$lang.availactions}</td>
                    <td >


                        <input type="submit" onclick="$('body').addLoader();"  name="create" value="Create" class="{if !$allowcreate}manumode{/if} btn btn-primary btn-sm" {if !$allowcreate && $details.manual!='1'}style="display:none"{/if}/>

                        <input type="submit"  name="suspend" value="Suspend" class="{if !$allowsuspend} manumode{/if} btn btn-default btn-sm" {if !$allowsuspend && $details.manual!='1'}style="display:none"{/if}  onclick="return confirm('{$lang.suspendconfirm}')"/>
                        <input type="submit" name="unsuspend" value="Unsuspend" class="{if !$allowunsuspend} manumode{/if} btn btn-default btn-sm" {if !$allowunsuspend && $details.manual!='1'}style="display:none"{/if}/>

                        <input type="submit" name="terminate"  value="Terminate" class="{if !$allowterminate}manumode{/if} btn btn-danger btn-sm" {if !$allowterminate && $details.manual!='1'}style="display:none;;"{else} style=""{/if} onclick="return confirm('{$lang.terminateconfirm}')"/>


                        <input type="submit" name="renewal"  value="Renewal" {if !$allowrenewal}style="display:none"{/if} class="btn btn-default btn-sm"/>
                        ...
                        {foreach from=$custom item=btn}
                            {if $btn!='GetOsTemplates' && $btn!='GetNodes' && $btn!='GetStatus' && $btn!='restoreBackup' && $btn!='createBackup' && $btn!='deleteBackup'}
                                <input type="submit" name="customfn" value="{$btn}" class="{if $details.status!='Active'}manumode{/if} {if $loadable[$btn]}toLoad{/if} btn btn-default btn-sm" {if $details.status!='Active' && $details.manual!='1'}style="display:none"{/if} />
                            {/if}
                        {/foreach}
                ...
                <tr><td>{$lang.sendacce}</td>
                    <td><select name="mail_id" id="mail_id">
                            {foreach from=$product_emails item=send_email}
                                <option value="{$send_email.id}" {if $send_email.id==$welcome_email_id}selected="selected"{/if}>{$send_email.tplname}</option>
                            {/foreach}
