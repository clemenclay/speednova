...
<ul class="leftmenu">
    ...
    {if $service.status=='Active'}
        ...
        <li {if $widget.name==$widg.name}class="current"{/if}><a  href="{$ca_url}clientarea/services/{$service.slug}/{$service.id}/&widget={$widg.name}{if $widg.id}&wid={$widg.id}{/if}"><span class="{$widg.name}">
                    <img src="{$system_url}{$widg.config.smallimg}" alt="" />{assign var=widg_name value="`$widg.name`_widget"}{if $lang[$widg_name]}{$lang[$widg_name]}{elseif $lang[$widg.name]}{$lang[$widg.name]}{elseif $widg.fullname}{$widg.fullname}{else}{$widg.name}{/if}</span></a></li>
        {/foreach}
    {/if}
