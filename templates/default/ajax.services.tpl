...
{foreach from=$services item=service name=foo}
 <tr {if $smarty.foreach.foo.index%2 == 0}class="even"{/if}>
  <td><a href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/"> {$service.catname} - <strong>{$service.name}</strong>  {include file='../common/tpl/inlinelabel.tpl'}</a>
...
<form action="" method="post">
	...
	<tbody><tr>
						...
						<input type="submit"  value="Rebuild OS" class="padded" onclick="$('#content').addLoader();"/>
...
<table width="100%" cellspacing="0" cellpadding="0" border="0" class="checker">
	   ...
	   <tr >
		...
		{/if}{if $service.domain!=''}
		 <tr class="even">
          <td  width="160" align="right">{$lang.hostname}</td>
