{if $affiliate}
    <div style="background:#E5EEF7;  margin: 0 0 10px; padding: 10px;">
...
{if $affiliate}
    {if $action == 'default'}
        ...
        {if $payout}
            <br />
            <center>
                <input type="button" class="padded"  value="{$lang.payout}" style="font-weight:bold;font-size:13px;" onclick="window.location.href='{$ca_url}tickets/new/';return false;"/>
            </center>
        {/if}	
        ...
        {if $integration_code!=''}
            <div>
                ...
                <div class="right">
                    <h3>{$lang.preview}</h3>
                    <iframe id="codepreview" onload="$(this).contents().find('body').html($('#codetext').val())" style="height: 100px; margin: 10px; width: 350px;"></iframe>
                </div>
                <div class="clear"></div>
            </div>
    ...
    {elseif $action == 'vouchers'}
        <div class="right">
            <input type="button" onclick="window.location.href='{$ca_url}{$cmd}/addvoucher/';return false;" value="{$lang.newvoucher}" >
        </div>
        <h3>{$lang.promocodes}</h3>
        
            <table cellspacing="0" cellpadding="0" border="0" width="100%" class="styled" style="text-align: center">
                <colgroup class="firstcol"></colgroup>
                <colgroup class="alternatecol"></colgroup>
    ...
    {elseif $action == 'addvoucher'}
        {if !$plan}
            ...
            <ul id="planlist"> 
                {foreach from=$commisionplans item=plan name=ul}
                    <li{if $smarty.foreach.ul.index % 2 != 0} class="even" {/if}>
                        <label><input style="vertical-align: text-bottom;" type="radio" name="plan" value="{$plan.id}" {if $smarty.foreach.ul.first}{/if} /> {$plan.name} 
                            {if $plan.type=='Percent'}{$plan.rate}%{else}{$plan.rate|price:$affiliate.currency_id}{/if}
                            {$lang.commission}
                        </label> 
                        , {$lang.appliesto} :
                        <span class="appliesto_short {if $plan.applicable_list.1}ext{/if}">
            ...
            {literal}
             <style type="text/css">
               ...
               #planlist li{padding:5px 0 7px; border-bottom: dotted 1px #dfdfdf}
               #planlist li.even{background:#f2f2f2}
               #planlist label{font-weight:bold;}
               #planlist .appliesto_short, #planlist .appliesto_full span{
               ...
               #planlist .appliesto_short.ext,  #planlist .appliesto_full, #planlist .ext{cursor:pointer}
               #planlist .appliesto_full {padding: 5px 0 0 5px;}
               #planlist .appliesto_full span{
                    display: block;
                    float: left;
               }
               #planlist .even .appliesto_short, #planlist .appliesto_full span {
        ...
        {else}
            ...
            <table width="100%" border=0 class="checker" cellspacing="0">
                ...
                <tr class="lastone">
                    <td colspan="6">
                    <center><input type="submit" value="{$lang.addvoucher}" name="save"></center>
                    </td>
                </tr>
...
{else}
    ...
    <div style="float:right">
        <img src="{$template_dir}images/money.png" alt="" />
    </div>

    <h1 style="margin:10px 0px;">{$lang.ycm} {if $config.AffType=='Percent'}{$config.AffValue}%{else}{$config.AffValue|price:$currency}{/if} {$lang.persale}</h1>
    ...
    <form action="" method="post">
        ...
        <center>
            <input type="submit" class="padded" style="font-weight:bold;font-size:16px;padding:15px 10px;" value="{$lang.becomeaffiliate}"/>
        </center>
        {securitytoken}
    </form>
    <br />
