...
<div class="clear"></div>
            <div style="padding:5px 20px 20px;">
                ...
                {foreach from=$services item=o}
                 <h3><input type="checkbox" class="privilege " id="smain_{$o.id}" onclick="cUnc(this,'s{$o.id}')" /> <label for="smain_{$o.id}">{$o.catname} - {$o.name} {if $o.domain}<em>( {$o.domain} )</em>{/if}{if $o.label} <i class="label label-default"> - {$o.label}</i>{/if}</label></h3>
                  <div class="pgroup">
                    <table border="0" cellspacing="0" cellpadding="0" width="100%">
                        <tr>
                            <td width="33%">
