{if $action=='details' || $action=='password' || $action=='ccard'  || $action=='ach'|| $action=='affiliates' || $action=='profilepassword' || $action=='ipaccess'}
<div class="wbox">
    <div class="wbox_header">
        ...
        <ul class="wbox_menu">
            ...
            {if $enableACH}
            <li {if $action=='ach'} class="bgon2"{else}class="bgoff2"{/if}><a href="{$ca_url}clientarea/ach/">{$lang.ACHecheck}</a></li>
            {/if}
	...
	{/foreach} {/if}
                            ...
                            {elseif $field.field_type=='Check'}
                            {foreach from=$field.default_value item=fa}
                            <input type="checkbox" name="{$k}[{$fa|escape}]" value="1" {if in_array($fa,$client[$k])}checked="checked"{/if} />{$fa}<br />
                            {/foreach}
                            {/if}
                            {/if}
                            {/if}
    ...
    <table width="100%" border=0 class="checker" cellspacing="0">
        <tbody>
            <tr><td align="right" width="160">{$lang.oldpass}</td>
                <td >
                    <input name="oldpassword" type="password" autocomplete="off"  class="styled"/>
                </td>
            </tr>
            ...
            <tr><td align="right">{$lang.confirmpassword}</td>
                <td >
                    <input name="password2" type="password" autocomplete="off"  class="styled"/><br>
                </td>
            </tr>
    ...
    <table width="100%" border="0" class="checker" cellspacing="0">
        ...
        <tr>
            <td colspan="2" align="center">
                {if $allow_storage}<input type="button" value="{$lang.changecc}" id="newccbut"  class="padded" onclick="$(this).attr('disabled','disabled'); $('#newccdetails').show(); return false;" /> {/if}
    ...
    <tr>
		...
		{if $action=='addfunds'}
            <div class="wbox">
				...
				{$lang.trans_amount}: <input name="funds" class="styled" size="4" value="{$mindeposit|price:$currency:false}"/> 
