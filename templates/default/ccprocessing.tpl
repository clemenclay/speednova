            ...
            {if $allow_storage || $ccard}
                <tr>
                    <td colspan="2"  class="even">
                        <input type="radio" name="cc_details" value="existing" {if $ccard}checked="checked"  onclick="$('#newcard').hide();"{else}disabled="disabled"{/if} /> 
                        {$lang.ccardexists} {if $ccard}({$ccard.cardnum}){/if}
                    </td>
                </tr>
                <tr class="even">
                    <td colspan="2">
                        <input type="radio" name="cc_details" value="new" {if !$ccard}checked="checked"{/if} onclick="$('#newcard').show();"/> {$lang.ccardnew}
                    </td>
                </tr>
            {/if}
            <tbody id="newcard" {if $ccard}style="display:none"{/if}>
                <tr>
                    <td align="right" width="160" >{$lang.ccardtype}</td><td>
                        {if !$allow_storage && !$ccard}
                ...
                <tr>
                    <td align="right">{$lang.ccardexp}</td>
                    <td>
                ...
                {if $allow_storage}
                    <tr>
                        <td align="right">{$lang.autosave_cc}</td>
                        <td align="left"><input type="checkbox" value="1" name="save_cc" {if !$ccard}checked="checked"{/if} /> {$lang.autosave_cc_desc}</td>
                    </tr>
                {/if}
            </tbody>
            <tr>
                <td align="right" width="160">{$lang.ccarcvv}</td>
                <td align="left">
                    <input type="text" name="cvv" size="4"  class="styled" autocomplete="off" style="width:60px"/> 
                    <a href="#newccdetails" data-toggle="modal">?</a>
                </td>
            </tr>
            <tr class="even">
                <td colspan="2" align="center">
                    <input type="submit" value="{$lang.continue}" name="continue"  class="btn btn-info" style="font-weight:bold"/>
                </td>
            </tr>
        </table>
    </div>
...
<div class="wbox">
	...
	<div class="wbox_content">
	 <table cellpadding="0" cellspacing="0" border="0"  class="checker" width="100%">
    <tbody>
