$.extend($.hbslider.effects, {
    blinds: {  //a.k.a "żaluzje"
        ...
        animate: function (direction,slider,settings) {
            ...
            var html = slider.slides.eq(slider.current).children().length?slider.slides.eq(slider.current).html():slider.slides.eq(slider.current).outerHTML();
            ...
            slider.slides.eq(slider.current).css({opacity:0});
