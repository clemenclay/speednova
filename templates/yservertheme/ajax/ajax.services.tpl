    ...
    {foreach from=$services item=service name=foo}
        <tr>
            <td class="overflow-elipsis">
                <div>
                    <a href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/" class="roll-link">
                        <span data-title="{$service.name}">{$service.name}</span> {include file='../common/tpl/inlinelabel.tpl'}
                    </a>
                    
                    {if $service.domain!=''}
                        <br/>
                        <a class="shared-link" href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/"> {$service.domain} </a>
                    {/if}
                </div>
            </td>
            <td class="bold"> <span class="label flat-ui-label label-{$service.status}">{$lang[$service.status]}</span></td>
            ...
            <td>
                {if $service.status=='Active' }
                    <div class="btn-group pull-right flat-ui-dropdown-button">
                        <a href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/" data-toggle="dropdown" data-contener="#page" class="btn btn-small btn-primary dropdown-toggle">
                            <i class="icon-cog"></i> 
                            <span class="caret"></span>
                        </a>
                        <ul class="dropdown-menu dropdown-left">
                            <li><a href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/label">{$lang.editlabel}</a></li>
                ...
                {else}
                    <a href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/" class="btn btn-small btn-primary pull-right">
                        <i class="icon-cog"></i>
                    </a>
                {/if}			
            </div>
...
{elseif ($action=='vps') && $services}
    {foreach from=$services item=service name=foo}
        <tr class="styled-row">
            ...
            <td class="p2">
                <div>
                    <a class="shared-hosting-title" href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/">{$service.name} {include file='../common/tpl/inlinelabel.tpl'}</a>
                </div>
            </td>
            <td class="label-{$service.status} bold">{$lang[$service.status]}</td>
