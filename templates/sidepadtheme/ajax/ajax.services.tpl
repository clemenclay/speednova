...
<tr {if $smarty.foreach.foo.index%2 == 0}class="even"{/if}>
    <td><a href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/">  <strong>{$service.name}</strong>  {include file='../common/tpl/inlinelabel.tpl'}</a>
    ...
    <td class="cell-border grey-c">{if $service.next_due!=0}{$service.next_due|dateformat:$date_format}{else}-{/if}</td>
...
{foreach from=$services item=service name=foo}
<tr {if $smarty.foreach.foo.index%2 == 0}class="even"{/if}>
    <td><a href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/"><strong>{$service.name}</strong>  {include file='../common/tpl/inlinelabel.tpl'}</a></td>
    ...
    <td align="center"><a href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/" class="view3">{$lang.view}</a></td>
</tr>
{/foreach}
{elseif  $customfile}
