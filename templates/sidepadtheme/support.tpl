       ...
       {/if}
          ...
          <div class="support-home-table">
              <div class="table-box">
                  <div class="table-header">
                      <div class="right-btns">
                          <a href="{$ca_url}tickets/" class="clearstyle btn grey-custom-btn">{$lang.more} <i class="icon-more-arrow"></i></a>
                      </div>
                  ...
                  <table class="table table-striped table-hover">
                  	{foreach from=$openedtickets item=ticket name=foo}
                        <tr>
                     	...
                     	<a href="{$ca_url}tickets/view/{$ticket.ticket_number}/" {if $ticket.client_read=='1'} class="no-bold" {/if}>
                                {$ticket.subject}
                        </a></td>
                        </tr>
                    {foreachelse}
...
<!--                  <ul class="nav nav-header s-home-list">
                  {foreach from=$openedtickets item=ticket name=foo}
                 	 <li><span class="label-{$ticket.status}">{$lang[$ticket.status]}</span>
                     	<a href="{$ca_url}tickets/view/{$ticket.ticket_number}/" {if $ticket.client_read=='1'} class="no-bold" {/if}>
                                {$ticket.subject}
                        </a>
                     </li>
