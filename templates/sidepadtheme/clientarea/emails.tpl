...
<div class="text-block clear clearfix">
    {if $email}
        <h5>{$email.subject}</h5>
        <div class="brcrm clear">
            <span class="annoucement_date left"><i class="icon-time"></i> {$lang.sent} {$email.date|dateformat:$date_format}</span>

            <a href="{$ca_url}clientarea/emails/"  class="btn btn-mini right"><i class="icon-chevron-left"></i> {$lang.back}</a>
    ...
    {else}
    <h5>{$lang.emails|capitalize}</h5>
    <p class="clear">{$lang.email_info}</p>

        {if $emails}

            <a href="{$ca_url}clientarea&amp;action=emails" id="currentlist" style="display:none" updater="#updater"></a>
            <table cellspacing="0" cellpadding="0" border="0" width="100%" class="table table-striped table-bordered fullscreen t-logs">
