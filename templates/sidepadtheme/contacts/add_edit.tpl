...
<h5 style="float:none;">{$lang.profiles}{if $action=='add'}: {$lang.addnewprofile} {else}: {$submit.firstname} {$submit.lastname}{/if}</h5>
<div class="brcrm">
...
<form method="post" action="" >
    ...
    <p class="add-contact pl19">{$lang.contactdetails} </p>
    ...
    <div class="brcrm">
        ...
        <div >
            ...
            <div id="priv_serv" {if $submit.privileges.services.full}style="display:none"{/if}>
                 ...
                 <h4><input type="checkbox" class="privilege " id="smain_{$o.id}" onclick="cUnc(this,'s{$o.id}')" /> <span for="smain_{$o.id}">{$o.catname} - {$o.name} {if $o.domain}<em>( {$o.domain} )</em>{/if}{if $o.label} <i> - {$o.label}</i>{/if}</span></h4>
                <div class="pgroup mb20">
                    <table border="0" cellspacing="0" cellpadding="0" width="100%">
                        <tr>
                            <td width="33%">
    ...
    <div class="form-actions">
        ...
        <input type="submit" value="{$lang.submit}"  class="clearstyle btn green-custom-btn h-btn" />
    </center>
          </div>
