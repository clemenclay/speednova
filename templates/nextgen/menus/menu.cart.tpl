            ...
            <div class="span8">
                <div class="row ">
                    <div class="span5">
                        <ul class="span5">
                            {if $shoppingcart}
                                <li class="nav-header span5">Items in cart</li>
                                    {foreach from=$shoppingcart item=order key=k}
                                        {if $order.product}
                                        <li>
                                            <a href="{$ca_url}cart&cart=edit&order={$k}">
                                                <span class="counter">{$order.total.total|price:$currency}</span>
                                    ...
                                    {elseif $order.domains }
                                        {foreach from=$order.domains item=domenka key=kk}
                                            <li>
                                                <a href="{$ca_url}cart&cart=edit&order={$k}">
                                                    <span class="counter">{$domenka.price|price:$currency}</span>
                                                    <span class="text-overflow">
                            ...
                            {else}
                                ...
                                <li class="span5">&nbsp;</li>
                                <li class="span5"><button class="btn btn-success" href="{$ca_url}cart/"><i class="icon-shopping-cart icon-white"></i> Place new order</button></li>
                                {/if}
                        </ul>
                    </div>
                    <div class="span3">
                        <center>
                            <div class="iconwrap"><span class="iconfont-price-tag iconfont-size5 silver" style=""></span><br></div>
                                {if $shoppingcart}
                                <p class="silver"></p>
                                <button class="btn btn-success" href="{$ca_url}cart/"><i class="icon-shopping-cart icon-white"></i> Proceed to cart</button>
                            {/if}
                        </center>
                    </div>
                </div>
