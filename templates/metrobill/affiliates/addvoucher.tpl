          ...
          <div class="tab-pane active">
            ...
            <form action="" method="POST" onsubmit="window.location.href = '{$ca_url}{$cmd}/{$action}/&plan=' + $(this).find('input[name=\'plan\']:checked').val();
                ...
                {foreach from=$commisionplans item=plan name=ul} 
                    <div class="commission-plan-box">
                        ...
                        <div class="commission-plan-applies">
                            ...
                            <p class="commission-plan-applicable">{$lang.appliesto}: 
                                {foreach from=$plan.applicable_list item=item name=commisionit}
                                    <span class="label label-info">{$item.cat}: {$item.name}</span>
                                {/foreach}
                            </p>
                ...
                <div class="form-actions">
                    <center><input type="submit" value="{$lang.continue}" class="btn btn-flat-primary" ></center>
                </div>
            </form>
            {literal}
                <style>
                ...
                $('.commission-plan-more').each(function() {
                    var self = $(this),
                        products = self.nextAll('.commission-plan-applicable').children();
                        
                    if(products.filter(':first').offset().top == products.filter(':last').offset().top){
                        self.remove();
                        return;
                    }
                    self.popover({html: true, content: self.next('.appliesto_full').html(), placement: 'left'}).click(function(){
                        $('.popover').not(self.next()).prev().popover('hide');
                    });
                });
                </script>
            {/literal}
            ...
            <form action="" method="POST">
                <table width="100%" border=0 class="table table-striped p-td" cellspacing="0">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="checkbox" name="max_usage_limit" onclick="check_i(this)" style="vertical-align: middle; margin:-5px 3px 0 0" />
                    ...
                    <tr class="even">
                        ...
                        <td>
                            ...
                            <input type="text"  name="expires" class="styled config_val haspicker" disabled="disabled"/>
                        </td>
                    </tr>
                    <tr>
                ...
                <div class="form-actions">
                    <center><input type="submit" value="{$lang.addvoucher}" name="save" class="btn btn-flat-primary"></center>
                </div>
                {securitytoken}
            </form>
            {literal}
...
<style type="text/css">
    ...
    a.dp-choose-date{background:url("{/literal}{$system_url}{literal}includes/libs/configoptions/datepicker/calendar.png") no-repeat;display:block;float:left;height:16px;margin:9px 8px 0;overflow:hidden;padding:0;text-indent:-2000px;width:16px}
