...
<li>
    <a {if $cmd == 'clientarea' && $action == 'default'} class="active"{/if} href="{$ca_url}clientarea/">
        <i class="ico-home"></i> <span>{$lang.dashboard}</span>
    </a>
</li>
<li class="cloud">
    <a href="{$ca_url}cart/" {if $cmd == 'cart'}class="active"{/if}>
        <i class="ico-cloud"></i> <span>{$lang.order}</span>
    </a>
    {include file='menus/menu.dropdown.cart.tpl'}
</li>
<li>
    <a href="{$ca_url}clientarea/services/" {if $cmd == 'clientarea' && ( $action == 'service' || $action == 'services' || $action == 'domains')} class="active"{/if}>
        <i class="ico-cog"></i>
...
<li>
    <a href="{$ca_url}clientarea/" {if $cmd == 'clientarea' && !( $action == 'service' || $action == 'services' || $action == 'domains') && $action != 'default'} class="active"{/if}>
        <i class="ico-user"></i> <span>{$lang.account}</span>
    </a>
...
<li>
    <a href="{$ca_url}{if $enableFeatures.kb!='off'}knowledgebase/{else}tickets/new/{/if}" {if $cmd == 'support' || $cmd == 'tickets' || $cmd == 'downloads' || $cmd == 'knowledgebase'} class="active" {/if}>
        <i class="ico-question-sign"></i> <span>{$lang.support}</span>
    </a>
...
{if $enableFeatures.affiliates!='off'}
    <li>
        <a href="{$ca_url}affiliates/" {if $cmd == 'affiliates'} class="active" {/if}>
            <i class="ico-pencil"></i> <span>{$lang.affiliates}</span>
        </a>
...
{foreach from=$HBaddons.client_mainmenu item=ad}
    <li>
        <a href="{$ca_url}{$ad.link}/"><i class="ico-cog"></i> <span>{$ad.name}</span></a>
    </li>
{/foreach}
