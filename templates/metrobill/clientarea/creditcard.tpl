...
<div class="sh-container">
            ...
            {if $ccard.cardnum}
                ...
                <form action="" method="post" style="margin-bottom:0px;">
                    {if $allow_storage}<a href="#newccdetails" data-toggle="modal" class="btn btn-flat-primary" >{$lang.changecc}</a>&nbsp;{/if}
                    {if $allowremove} 
                        <button class="btn btn-flat-danger" type="submit" name="removecard" onclick="return confirm('{$lang.removecc_confirm}')" >
                            {$lang.removecc}
                        </button>
                    {/if}
                    {securitytoken}
        ...
        {else}
            ...
            <a href="#newccdetails" class="btn btn-flat-primary" data-toggle="modal"><i class="icon-add"></i> {$lang.newcc}</a>
        {/if}  
        <div id="newccdetails" style="display:none" class="modal">
            <form action="" method="post" style="margin-bottom:0px;">
                ...
                <div class="modal-footer">
                    <a href="#" class="btn btn-flat-warning" data-dismiss="modal">{$lang.close}</a>
                    <button type="submit" name="addcard"  class="btn btn-flat-primary">
                        {$lang.savechanges}
                    </button>
                </div>
