...
{elseif ($action=='services'  || $action=='reseller' || $action=='servers' || $action=='accounts') && $services}
    {foreach from=$services item=service name=foo}
        ...
        <tr>
            ...
            {if $service.domain!=''}
                <td class="table-two-rows overflow-elipsis">
                    <a class="bold blue-txt" href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/">{$service.name} {include file='../common/tpl/inlinelabel.tpl'}</a>
                    {if $service.domain!=''}<a class="blue-txt" href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/">{$service.domain}</a>
                    {/if}
                </td>
            {else}
            ...
            <td class="fix-p align-r">
                {if $service.status=='Active' }
                    <div class="btn-group">
                        <a class="btn btn-white small-p dropdown-toggle" data-toggle="dropdown" href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/">
                            <i class="icon-c-cog"></i>
                            <span class="caret"></span>
                        </a>
                        <ul class="dropdown-menu c-dropdown-menu menu-right">
                            <li><a href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/label" style="color:#737373">{$lang.editlabel}</a></li>
                ...
                {else}
                    <a href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/" class="btn btn-white small-p">
                        <i class="icon-c-cog"></i>
                    </a>
                {/if}
            </td>
