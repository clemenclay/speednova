        ...
        that.next('ul').each(function(){ 
            ...
            if(name != undefined && name.length && !$('select[name="'+name+'"].dropdown').length){
                ...
                ul.find('a').each(function(){
                    ...
                    if(val.length > 1){
                        ...
                        a.click(function(){
                            ...
                            if(itemrel) {
                                toggleval(itemrel,a.attr('href').substr(1));
                ...
                if(itemtg &&  that.attr('href') != undefined && that.attr('href').substr(1).length)
                    toggle(itemtg);
                if(itemrel &&  that.attr('href') != undefined && that.attr('href').substr(1).length) {
                    toggleval(itemrel,that.attr('href').substr(1));
                }
            }
...
function toggleval(name,val) {
    var inp = $('input[name="'+name+'"]').length;
    if(!inp){
        $('<input name="'+name+'" type="hidden" value="'+val+'" />').appendTo('#cartforms');
    }
}
