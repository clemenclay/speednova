        ...
        <div class="option-list" style="display:none" id="excol_{$cf.id}">
            ...
            <ul>
                ...
                {elseif $cf.type=='select'}
                       <li ><select name="custom[{$cf.id}]" id="custom_field_{$cf.id}"  class="pconfig2_ custom_field_{$cf.id}" {if $cf.config.conditionals}onchange="$('.custom_field_{$cf.id}').fieldLogic();"{/if}>
                    {foreach from=$cf.items item=cit name=fconf}

                            <option value="{$cit.id}"  {if $cart_contents[1]}{if $cart_contents[1][$cf.id][$cit.id]}selected="selected"{/if}{elseif $smarty.foreach.fconf.first}selected="selected"{/if} >
                            {$cit.name} {if $cit.price!=0} (  + {$cit.price|price:$currency}){/if}
                            </option>
                    {/foreach}
                    </select>    </li>
