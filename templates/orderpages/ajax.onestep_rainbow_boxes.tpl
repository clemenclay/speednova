...
<div id="billingCycle" class="billing-method">
    {$lang.pickcycle}
...
<div class="promo-code-box clearfix">
    {if !$subtotal.coupon}
        <div class="promo-code-field">
            <input name="promocode" placeholder="{$lang.usecoupon}" type="text">
        </div>
        <button onclick="applyCoupon()" class="btn btn-promo-code">OK</button>
    {/if}
</div>
...
{if $tos}
    <div class="tos-box">
        <p><input id="checkbox-tos" type="checkbox" name="tos2" /> {$lang.tos1} <a href="{$tos}" target="_blank">{$lang.tos2}</a></p>
    </div>
{/if}

<button onclick="$('#orderform').submit()" class="btn btn-block btn-order">Checkout</button>
    ...
    <form id="cartforms" action="" method="post">
                    ...
                    <div class="span4">
                        {foreach from=$addons item=a key=k}
                            <div class="configuration-box">
                                ...
                                <div class="padding">
                                    <div class="option-val subproduct-val">
                ...
                {if $subproducts}
                    <div class="span4">
                        {foreach from=$subproducts item=a key=k}
                            <div class="configuration-box">
                                <div class="configuration-header dark">
                                    <h4>{$a.category_name}</h4>
                                </div>
                                <div class="padding">
                                    <div class="option-val subproduct-val">
                                        <p><input name="subproducts[{$k}]" onchange="if (typeof(simulateCart)=='function')simulateCart('#cart0');"  type="checkbox" value="1" {if $contents[4].$k}checked="checked"{/if}/> <span> {$a.name}</span></p>
                                            {price product=$a}
                                            {if $a.paytype=='Free'}
                                            <span class="product-cycle cycle-free">{$lang.free}</span>
        ...
        {if ($product.hostname) && !($cart_contents[0].domainoptions.register=='1' ||  $allowown || $subdomain)}
            <div class="row">
                <div class="span9">
                    <div class="configuration-box">
                        <div class="configuration-header green">
                            <h4>{$lang.hostname}</h4>
                        </div>
                            ...
                            <div class="tab-content">
                                <div class="tab-pane active" id="home">
                                    <div class="padding clearfix">
                                        <p>{$lang.hostname}</p>
                                        <input type="text" name="domain" value="{$item.domain}" class="styled" size="50" onchange="if(typeof simulateCart == 'function') simulateCart();"/>
                                    </div>
                                </div>
                            </div>
                            <ul class="nav nav-tabs domain-nav" id="domainTab">
                                <li class="active"><a href="#home">{$lang.hostname}</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <div id="domoptions11" class="domain-tab">
                    <div class="padding clearfix">
                        ...
                        <div class="tab-content">
                            ...
                            <!-- Own -->
                            {if $allowown}
                                <div class="tab-pane" {if $contents[2]}style="display: none"{/if}  id="own">
                    ...
                    <ul class="nav nav-tabs domain-nav" id="domainTab">
                        {if $allowregister}
                            <li class="active"><a data-toggle="tab" href="#illregister" onclick="setAction(this);">{$lang.register}</a></li>
                        {/if}
                        {if $allowtransfer}
                            <li><a data-toggle="tab" href="#transfer" onclick="setAction(this);">{$lang.transfer}</a></li>
                        {/if}
                        {if $allowown}
                            <li><a data-toggle="tab" href="#own" onclick="setAction(this);">{$lang.alreadyhave}</a></li>
                        {/if}
                        {if $subdomain}
        ...
        <div class="row custom-forms">
            {counter name=cfcount print=false assign=cfcounter start=0}
            ...
            {foreach from=$custom item=cf}
                ...
                {if $cf.items}
                    ...
                    {elseif $cf.type == 'slider' && $slidersnow > 2}
                        {foreach from=$cf.items item=cit}
                            <div class="span4">
                                <div class="configuration-box">
                                    <div class="configuration-header dark">
                                        <h4>{$cf.name} {if $cf.options &1}*{/if}</h4>
                                    </div>
                                    <div class="padding">
                                        <div class="cf-slider">
                                            <input
                                                    ...
                                                    />
                                            <p><strong>{if $contents[1][$cf.id][$cit.id].qty}{$contents[1][$cf.id][$cit.id].qty}{else}{$cf.config.initialval}{/if}</strong>
                                                <span>{if $contents[1][$cf.id][$cit.id].price}({$currency.sign} { $contents[1][$cf.id][$cit.id].price} {$currency.code}){/if}</span>
                                            </p>
                                            <div id="slider-{$slidersnow}" class="slider-path"></div>
                            ...
                            <script type="text/javascript">
                                _normalSlider.push( {literal}{{/literal}
                    ...
                    {else}
                        <div class="span4">
                            <div class="configuration-box">
                                <div class="configuration-header box-color-{$cfcounter%4}">
                                    <h4>{$cf.name} {if $cf.options &1}*{/if}</h4>
                                </div>
                                <div class="padding">
