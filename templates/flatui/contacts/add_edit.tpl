...
{include file="clientarea/leftnavigation.tpl"}
	...
	<div class="sh-container">
            ...
            <form class="m20" action='' method='post'>
                ...
                <div class="m15">
                    <button type="submit" class="btn btn-flat-primary"> {$lang.submit} </button>
                </div>
                <h4 class="mt-50">{$lang.privileges}</h4>

                <p><b>{$lang.premadepriv}</b>
                    ...
                    {foreach from=$premade item=p}
                        <a href="#" rel="{$p.file}" onclick="return loadProfile($(this).attr('rel'))" class="label flat-ui-label label-info">{if $lang[$p.lang]}{$lang[$p.lang]}{else}{$p.name}{/if}</a>
                    {/foreach}
                </p>
                <div class="clear"></div>
                ...
                <div >
                    ...
                    <div id="priv_serv" {if $submit.privileges.services.full}style="display:none"{/if}>
                        {foreach from=$services item=o}
                            <br />
                            <strong><input type="checkbox" class="privilege " id="smain_{$o.id}" onclick="cUnc(this, 's{$o.id}')" /> <span for="smain_{$o.id}">{$o.catname} - {$o.name} {if $o.domain}<em>( {$o.domain} )</em>{/if}{if $o.label} <i class="label label-default">{$o.label}</i>{/if}</span></strong>
                            <div class="pgroup mb-20">
                                <table border="0" cellspacing="0" cellpadding="0" width="100%">
                                    <tr>
                                        <td width="33%">
