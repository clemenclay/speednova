    ...
    <div class="brcrm">
        ...
        <div >
            ...
            <div id="priv_serv" {if $submit.privileges.services.full}style="display:none"{/if}>
                 ...
                 <h4><input type="checkbox" class="privilege " id="smain_{$o.id}" onclick="cUnc(this,'s{$o.id}')" /> <span for="smain_{$o.id}">{$o.catname} - {$o.name} {if $o.domain}<em>( {$o.domain} )</em>{/if}{if $o.label} <i class="label label-default">{$o.label}</i>{/if}</span></h4>
                <div class="pgroup mb20">
                    <table border="0" cellspacing="0" cellpadding="0" width="100%">
                        <tr>
                            <td width="33%">
